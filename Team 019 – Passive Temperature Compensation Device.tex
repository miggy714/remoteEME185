...
\section*{Design Approach}
    ...
    Two subsystems accomplish this objective. The Actuator thermomechanically converts ambient temperature changes to displacement; the Valve restricts gas flow as driven by the actuator.
    To meet the above required relationship, each subsystem must comprise a simple architecture with a single degree of freedom and yield a first-order output with a narrow margin for error.
    Most notably, each of these must exhibit a linear input-output behavior, an unnatural property for flow-control valves.
    Motivated by these specifications, the \device\ design combines aerospace-standard technology with a novel valve architecture to convert thermal expansion to flow control.
    A bimetallic actuator directly translates ambient temperature changes to displacement of the valve's orifice component;
    the valve, in turn, contains and restricts the high-pressure flow with a variable diamond orifice, composed of the overlap between two sharp-cornered holes.
...
\begin{equation}
    \begin{aligned}
        ...
        &= \left[ \left( \specOrigActuatorLength \right) \left( \specActuatorCTE \right) - \left( \specMountEffLength \right) \left( \specMountCTE \right) \right] = \SI{1.91}{\micro\meter\per\celsius}
    \end{aligned}
\end{equation}
