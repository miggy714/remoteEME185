...
<<<<<<< HEAD
The functional core of the \device\ is a bimetallic actuator that leverages the difference in Coefficients of Thermal Expansion (CTE) of Aluminum 6061 and ALLVAR. While standard metals expand when heated, the ALLVAR alloy is engineered with a negative CTE ($\alpha \approx \specMountCTE$), meaning it physically contracts as temperature increases. 

By fixing an Aluminum actuator ($\alpha \approx \specActuatorCTE$) within an ALLVAR mounting structure, the \device\ transforms ambient temperature fluctuations into precise mechanical work. As the temperature rises, the Aluminum core pushes downward while the ALLVAR housing pulls the reference plane upward\todo{Actually since CTE<0, it also pushes the ref plane downward}. This "push-pull" effect creates a compounded relative
 displacement at the valve stem. This thermo-mechanical coupling allows the device to achieve the high-resolution motion required to maintain the $\pm 13$ \textmu m orifice tolerance without any external power or electronic feedback. This relationship is mathematically modeled by the governing equation $\govActuator$\todo{Better eqn: $\Delta L = \left( L_{core} \alpha_{core} - L_{mount} \alpha_{mount} \right) \Delta T$}, where the effective displacement is a direct 
 function of the material properties and the thermal gradient.

At the heart of the design is the valve orifice, which is\todo{comprises} a square\todo{diamond} aperture that provides a linear flow response to the actuator's motion. 
Two square holes form the aperture, one in the valve outlet and one in the valve stem, which are precisely aligned to create an effective 
...
relationship between actuator displacement and flow restriction (Appendix~\ref{app:derivations}). The orifice is cut using wire EDM, allowing 
for a very small minimum corner radius (0.056 mm), which is critical for achieving the required precision in flow control. 
The valve orifice extends into two stems that transfer the actuator's motion to the aperture. All contact surfaces are coated with a \todo{noun} to prevent galling.

A bimetallic actuator is fixed above the housing and welded to the valve stem; it drives the central orifice as it responds to temperature 
changes. The outer material, ALLVAR, shrinks when heated, while the inner material, Aluminum, expands \cite{ref_muraca1972aluminum}. Thus, as the ambient temperature 
increases, the actuator drives the valve stem downwards, shrinking the orifice. As the temperature decreases, the actuator pulls the stem 
back up, opening the orifice again. Across the total design temperature range, the actuator expands \SI{267}{\micro\meter}, driving the valve 
=======
...
>>>>>>> 036e7054bdc711624c4c2f54d862e1269ee01635
an equal distance.
...
Calibration of the device is performed during final assembly through the use of a manually-torqued adjustment screw and a Belleville washer. By adjusting 
