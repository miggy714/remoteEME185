...
By fixing an Aluminum actuator ($\alpha \approx \specActuatorCTE$) within an ALLVAR mounting structure, the \device\ transforms ambient temperature fluctuations into precise mechanical work. As the temperature rises, the Aluminum core pushes downward while the ALLVAR housing pulls the reference plane upward. This "push-pull" effect creates a compounded relative
 displacement at the valve stem. This thermo-mechanical coupling allows the device to achieve the high-resolution motion required to maintain the $\pm 13$ \textmu m orifice tolerance without any external power or electronic feedback. This relationship is mathematically modeled by the governing equation $\govActuator$, where the effective displacement is a direct 
 function of the material properties and the thermal gradient.

At the heart of the design is the valve orifice, which is a square aperture that provides a linear flow response to the actuator's motion. 
...
The valve orifice extends into two stems that transfer the actuator's motion to the aperture. All contact surfaces are coated with a to prevent galling.
