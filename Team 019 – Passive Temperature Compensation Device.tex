...
\section*{Problem Statement}
    ...
    The \device\ must maintain this orifice, no larger than the tip of a ballpoint pen, to within a hair's breadth of its setpoint across a wide temperature range.
...
\end{figure}
    ...
    Aluminum's expansion rate $\alpha_{\text{Al}} \approx \specActuatorCTE$; it expands when heated. ALLVAR, a proprietary technology made by ALLVAR Alloys, shrinks when heated at a rate of $\alpha_{\text{ALLV}} \approx \specMountCTE$.
    ...
    The assembly linearly translates the ambient temperature into compount relative displacement according to the governing equation $\Delta L = \left( L_{\text{core}} \alpha_{\text{Al}} - L_{\text{mount}} \alpha_{\text{ALLV}} \right) \Delta T$.
    ...
    displacement at the valve orifice. This relationship is mathematically modeled by the governing equation  $\Delta L = \left( L_{actuator} \alpha_{actuator} - L_{mount} \alpha_{mount} \right) \Delta T$, where the effective displacement is a direct  
    ...
    While most gas flow control valves exhibit a first-order change in orifice area, the required temperature-diameter translation required that the orifice remain a regular polygon.
    ...
    Their \SI{45}{\degree} rotation allows the orifice to remain a square, resulting in the linear output required.
    ...
    The orifice is cut using wire electro discharge machining (EDM), allowing 
...
The \device\ orifice is the diamond-shaped overlap between two $45^\circ$-rotated squares with area $A = s^2$ where $s$ is the inscribed square side.
