...
When desgining our system, the first major hurdle that we had to overcome was getting our system to linearly actuate across the entire 
temperature range. In coming up with ideas to accomplish this, we split the system into two subsystems. Then all we would need to do was 
make sure that each subsystem was linear, and the overall system would be linear. For both subsystems, we looked over many differnet design
possibilities. 
...
\chapter{DELETE BEFORE SUBMISSION: Brainstorm -- what sets our design apart?}

	\begin{itemize}
			\item Square orifice
			\begin{itemize}
					\item Linear flow response
					\item Simple, minimal DOF
					\item Cut with sinker EDM: very small corner radius (wire EDM minimum $\approx \SI{0.056}{mm}$)
			\end{itemize}

			\item Bimetallic actuator
			\begin{itemize}
					\item Compact form factor
					\item Solid construction is geometrically simple
					\item Constant coefficient of thermal expansion
					\item Thermally isolated from surroundings
			\end{itemize}

			\item Weld joints
			\begin{itemize}
					\item Enables simple mates with minimal degrees of freedom
			\end{itemize}

			\item Calibration
			\begin{itemize}
					\item Eliminates constant offset errors from manufacturing or assembly
					\item Procedure is straightforward to perform
			\end{itemize}

			\item Bellows
			\begin{itemize}
					\item Small, flexible, high-pressure seal
					\item Hydrogen-compatible
					\item Low reaction force
					\item Provides valve stem support
					\item Housing dimensioned to match convolution OD and length exactly
			\end{itemize}

			\item Direct interface
			\begin{itemize}
					\item Trade-off analysis performed: precision prioritized over height
					\item Client communication conducted to adjust design priorities
			\end{itemize}

			\item Material selection
			\begin{itemize}
					\item Invar 36: low CTE, justified via Ashby plot
					\item ALLVAR: negative CTE alloy, contracts with increasing temperature
					\item Aluminum 6061: selected per client input (confirm rationale with Isaiah)
					\item Inconel 718 bellows: high strength at elevated temperature
			\end{itemize}

			\item Precision fits
			\begin{itemize}
					\item Exact interference and clearance fits specified throughout assembly
			\end{itemize}

			\item Valve orifice coating
			\begin{itemize}
					\item Surface coating applied to prevent galling
			\end{itemize}
	\end{itemize}
