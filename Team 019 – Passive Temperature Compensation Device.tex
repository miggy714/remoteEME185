...
Then we made sure that both of them were linearly dependent on the ambient temperature and if they were both individually linearly dependent then combined they would still remain linearly dependent. When coming
up with the design for both the actuator and the valve we went through many different possibilities, but eventually decided on the design we did using a scoring matrix to rank all of the designs we had a see which one 
fit our requirements the best.

For the actuator, some of the designs we came up with were: bimetallic coil, thermal expansion rod, wax bladder, piston-cylinder assembly. The criteria we used when evaluating these different designs were: effectiveness over the 
required temperature range, linearity of response, fit within required mechanical envelope, ability to withstand fluid force and adequate displacement so we would not need a gear mechanism to amplify the motion. After evaluating all of
the different designs we decided on a variant of the thermal expansion rod due to it's simplicity in design and it's very predictable linear response to temperature changes. Then we needed to choose the material for the actuator. For this we 
went with aluminum due to its high coefficient of thermal expansion, low cost, and ease of manufacturing. We also needed to decide on the material for the mount that held the actuator. At first we went with steel due to it's low cost and 
machinability, but we found that steel's coefficient of thermal expansion
