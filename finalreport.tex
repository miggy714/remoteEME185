...
\label{chap:design}
	...
	\label{sec:design:just}
		...
		\label{subsec:design:just:geo_sel}
			...
			in section \ref{subsec:intro:specs:RR}. $A_1$ is expressly first-order. Any nonlear behavior in one subsystem must be either reparameterized in the other subsystem, an imprecise process involving a complex mechanical interface, or tolerated error.
			$\hat{G}_{RR}$'s tolerable error is limited to 5\% of $A_1$, or $\SI{76.2}{\frac{nm}{\degC}}$, a razor-thin margin.
			To best reach this target, the selection process only permitted subsystem architectures that exhibited first-order behavior, allowed minimal degrees of freedom, and interfaced directly with one another.
			Thus, the final design contains a single moving part, and the actuator and valve stem are precision-welded directly against one another.
			
			Many actuator design alternatives readily fit these criteria, as most materials maintain a linear thermal expansion rate.
			However, most valve architectures more readily translate their driving motion directly to the area of their orifice area rather than their effective diameter.
			...
			Although it was an entirely novel approach to this problem, the design fit all three selection criteria and, as demonstrated in chapter \ref{chap:sat}, enabled satisfaction of the Required Relationship.
		...
		\label{subsec:design:just:bellows}
			...
			The valve stem, which passes through the bellows, measures just smaller than the convolution inner diameter (ID) of the bellows to provide support against the strong external pressure.
			These both measure \specBellowsID.
			
		\subsection{Noncritical dimensions and features}
		\label{subsec:design:just:gen_dim}

			\todo{Justify all noncritical dimensions here (act diameter enough for minimal compression, internal channel size and height and valve stem diameter just big enough for bellows, pipe-interface thread size and length). Talk about machinability, assemblability (e.g. breaking the Housing Center up like we did so we could weld bellows). Show satisfaction of valve ME spec; justify violation of actuator ME spec.}
			...
			Here, the noncritical elements are justified as motivated by our design specifications.

			\subsubsection{Actuator Diameter}
			\label{subsubsec:design:just:gen_dim:act_diam}

				The bellows act as springs.
				Applying a slight resistance force as the valve orifice moves, they compress or stretch the long, thin metal of the two actuator rods, which themselves act as springs in series and compound one another's error as the deviate from their intended length.
				Though imperceptable, this deflection can be easily minimized and optimized by increasing the cross-sectional area of each component properly.
				As calculated later in sections \ref{subsec:sat:TF:force_balance:M} and \ref{subsubsec:sat:TF:force_balance:A}, their areas are nearly equal, ensuring minimal net deflection, as shown in Figure \ref{fig:mount_thick_opt}.
				The deflection decrease reaches a point of diminishing return at a total outer diameter (OD) of around 25 mm (Figure \ref{fig:mount_OD_opt}), and the mount's wall thickness was optimized at 3.5 mm (Figure \ref{fig:mount_thick_opt}).
				The total cross-section area was maximized by precisely fitting the two cylinders together, as further described in Section \ref{subsec:design:just:fits}.
				...
				\begin{figure}[H]
					...
					\begin{tikzpicture}
						...
						\end{axis}
					\end{tikzpicture}
					\caption{Actuator subsystem OD optimization}
			...
			\subsubsection{Housing Inlet Dimensions}
			\label{subsubsec:design:just:gen_dim:HI}

				The bellows (Appendix \ref{app:datasheets}) primarily drove the valve's internal geometry and total height.
				Each bellows measures just under 7 mm in diameter and \specBellowsHeight\ tall. The Housing Inlet contains a channel just wide enough for each bellows to fit snugly inside, minimizing empty internal space.
				The valve orifice measures exactly 7 mm so it slides precisely inside this channel.

				Each housing center provides a 10 mm wall to withstand the pressure force. Pressure calculations are dealt with in section \ref{subsec:design:just:P}.

				Two bellows, a 5 mm valve orifice, and two 10 mm walls total

				\begin{equation}
					2 (22.86) + 5 + 2 (10) = 70.72\ mm
				\end{equation}

				This violation of the vertical mechanical envelope is addressed in section \ref{subsubsec:design:just:gen_dim:ME}.

			\subsubsection{Threaded Interface}
			\label{subsubsec:design:just:gen_dim:thread}

				\todo{3/4" thread and required length for it on either side of the housing. Extra 10mm on housing outlet to bear force}

			\subsubsection{Machinability Considerations}
			\label{subsubsec:design:just:gen_dim:machine}

				\todo{Any features we changesd for machinability}

			\subsubsection{Assemblability Considerations}
			\label{subsubsec:design:just:gen_dim:assem}

				\todo{Housing center in pieces to enable weld; briefly walk through whole process and how design accommodates}
			...
			\label{subsubsec:design:just:gen_dim:ME}
				...
				\todo{Demonstrate valve compliance; justify actuator violation for the sake of precision -- verified priority shift with sponsor (shows good communication)}
...
\begin{thebibliography}{99}
	...
	\bibitem{ref_miniflex_design}
	Mini-Flex Corporation, \textit{Metal Bellows Design Guide}, p. 5, Mini-Flex Corporation, Oxnard, CA. [Online]. Available: \url{https://mini-flex.com/metal-bellows-design-guide/}
