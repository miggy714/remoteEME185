...
\label{chap:Fintro}
	...
	\label{sec:intro:motive}
		...
		\label{subsec:intro:motive:background}
			...
			Sandia National Laboratories, a United States security research facility in Livermore, CA, performs mayriad experiments that require precision flow control of high-pressure $\text{H}_{\text{2}}$ gas.
			These experiments are sensitive to environmental temperature fluctuations and must be tightly controlled.
			Gas flow in one or two independent channels must be restricted by an orifice a few dozen micrometers across at most that closes as the ambient air temperature rises to protect sensitive equipment or carefully control conditions downstream.
...
\label{chap:concl}
	...
	\label{sec:conc:fin_anal}
		...
		Gas flow experiments conducted by Sandia National Labs depend on submillimeter-precision Hydrogen flow control without the luxury of electronics or controls,
		handicapping the United States' competetive edge in the international theater.
		Existing technology provides a foundation for thermomechanical flow control but fails to approach the range, precision, or pressure capacity these experiments require.
		Without the \device, Sandia depends on costly active-control mechanisms and must conduct numerous experiments with substantial post-processing.
		Completion of the \device\ will allow them to significantly simplify the data collection and interpretation process, eliminating uncertain variables and improving the reliability of their results.

		The proof-of-concept Passive Temperature Compensation Device presented in this Final Design Report provides Sandia with the necessary foundation for development of a working prototype.
		The combination of proven, aerospace-grade components (the ALLVAR-Aluminum actuator, flexible metal bellows, belleville calibration, and electron-beam welding)
		with the novel diamond-orifice valve accomplishes previously impossible thermomechanical gas flow control.
		In the time provided for development of this device, only the first iteration could be produced.
		Risk analysis in chapter \ref{chap:comp} demonstrated that certain issues will need to be researched and certain revisions made before the \device\ can accomplish its objectives without failure.
		However, these adjustments are minor will likely not require fundamental revision of the design architecture.
		While this report does not provide a finished product, it lays a complete design architecture foundation, demonstrates its viability as a functional device, and provides convenient tools for iterative improvement and verification.
		The exact output relationship of the \device\ holds little value for any particular experiment, but the selected architecture allows for easy reparameterization on a case-by-case basis
		to accommodate individual experiments. Fine adjustment of the actuator, mount, and valve stem lengths allows for a fully customizeable Required Relationship for further iterations. 

	\section{Open Questions and Recommended Improvements}
	\label{sec:conc:rec}

		Certain assumptions remain unaddressed that could not be sufficiently covered in the DFMEA analysis. The following provides a starting point for iterative improvement of the current design.
		...
		\begin{itemize}
			...
			\item As mentioned in section \ref{subsubsec:design:just:gen_dim:ME}, certain dimensions can be optimized to decrease the Mechanical Envelope if necessary.
				The verification tools developed in chapters \ref{chap:sat} and \ref{chap:comp} provide a convenient method to quantify the impact of these changes on system output precision.
				\begin{itemize}
				...
				\end{itemize}
			\item Orifice dimensions can also be optimized. The side lengths of each square and, more importantly, the \SI{1}{\milli\meter} thickness of each orifice wall, were arbitrarily chosed to provide easy-to-cut features.
...
\label{app:drawings}
	...
	% \includepdf[pages=1-12, fitpaper=true, landscape=true]{pdf/05182026_DrawingPackage_v31.pdf}
...
% \includepdf[pages=1, fitpaper=true, landscape=true]{pdf/FinalGanttChart.pdf}
