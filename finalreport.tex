...
\definecolor{sandialb}{RGB}{0,  174, 214}
...
\label{chap:Fintro}
	...
	\label{sec:intro:specs}
		...
		\label{subsec:intro:specs:ME}
			...
			\todo{Fix the float issues with these boxes}
...
\label{chap:design}
	...
	\label{sec:design:just}
		...
		\end{equation}
			...
			Electron Beam Welding (LBW) is used for joint construction with a square butt joint geometry to ensure 
...
\label{chap:sat}
	...
	\label{sec:sat:error}
		...
		\label{subsec:sat:error:mach}
			...
			The values represented in the engineering drawings (Appendix \ref{app:drawings}) are taken from HAAS VF-2 series specifications \cite{haas_vf2_specs}. \todo{Cite gen eng drawing tolerances. HAAS?}
	...
	\label{sec:sat:TF}
		...
		Its value can be approximated as a first-order Taylor Expansion\cite{ref_Jensen}\cite{ref_Stewart} with very little error due to the linear behavior of thermal expansion. This yields
			...
			where the material expansion coefficients are shown in Appendix \ref{app:calcs:TF:G_therm}:
    ...
    \label{subsec:sat:TF:orif}
			...
			\label{subsubsec:sat:TF:orif:G_D}
				...
				\begin{equation}
					D_{eff} = \sqrt{\frac{4}{\pi} A} = \sqrt{\frac{4 X^2}{2 \pi}} = \sqrt{\frac{2}{\pi}} X
				\end{equation}
		...
		\label{sec:sat:TF:results}
			...
			Because this combination is unlikely to occur, a uniform-distribution Monte Carlo analysis\cite{ref_figliola} yields an informative probabilistic normalized distribution of net propagated error.
			
			Iterating through $\Tcold \leq T_{amb} \leq \Thot$ and plotting the results, this yields Figure \ref{fig:TF_res}.
			...
			\begin{figure}[h]
					...
					%% ── Transfer function low (worst-case −) ──────────────────────────────────
					\addplot [
													name path   = TF_low,
													color       = ucdgold,
													line width  = 1.6pt,
					...
					%% ── Legend entries for TF bounds ──────────────────────────────────────────
					\addplot [color=ucdgold, line width=1.6pt, dash dot]
													coordinates {(-200,0)};
					\addlegendentry{Worst-case output (high)}

					\addplot [color=ucdgold, line width=1.6pt, dashed]
													coordinates {(-200,0)};
					\addlegendentry{Worst-case output (low)}
			...
			Monte Carlo analysis produced the probability destribution of uncalibrated deviations from the nominal effective diameter as a percentage of the tolerance window at $T_{ref}$.
			...
			\begin{figure}[h]
					...
					\addplot [
													...
													y           = pct_err,
													col sep     = comma,
					] {figures3/error_dist.csv};

					%% ── Normal PDF overlay ────────────────────────────────────────────────────
					%% mu = -46.77, sigma = 86.45  (fitted from data)
					\addplot [
													color       = ucdgold,
													line width  = 1.6pt,
													solid,
													domain      = -450:350,
													samples     = 300,
					] {1/(86.45*sqrt(2*pi)) * exp(-((x-(-46.77))^2)/(2*86.45^2))};
					\addlegendentry{$\mathcal{N}(\mu{=}{-}46.77,\;\sigma{=}86.45)$}

					%% ── Histogram legend proxy ────────────────────────────────────────────────
					\addplot [
													fill        = sandiablue!30,
													draw        = sandiablue!60,
													line width  = 0.3pt,
					...
					\addlegendentry{Empirical distribution}

					\end{axis}
					\end{tikzpicture}%
					\caption{Percentage error distribution with fitted normal curve
									($\mu = -46.77$\%, $\sigma = 86.45$\%, $n = 10^6$).}
...
\label{app:calcs}
	...
	\label{app:calcs:cal}
		...
		\label{app:calcs:cal:sat}
			...
			\begin{equation}
				D_{eff}(T_{ref}) = 292.1 \pm \SI{12.7}{\micro\meter} = \SI{25.4}{\micro\meter} \text{ total range}
			\end{equation}
