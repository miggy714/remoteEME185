...
\label{chap:design}
	...
	\label{sec:design:just}
		...
		\label{subsec:design:just:LBW}
			...
			The axial stress on the weld is calculated as the force from the internal pressure distributed across the weld area \cite[eq.~1-6]{ref_Hibbeler}.
...
\label{chap:concl}
	...
	\label{sec:conc:fin_anal}
		...
		Without the \device, Sandia must either depend on active-control mechanisms, introducing an unnecessary variable in their regulation of experimental parameters,
...
\label{app:calcs}
	...
	\label{app:calcs:cal}
		...
		\label{app:calcs:cal:force}
			...
			which cites DIN EN 16983\cite{ref_dinen16983} and 16984\cite{ref_dinen16984} for their spring properties. DIN EN\footnote{i.e. English} 16984 in turn cites a 1936 paper by Almen and Laszlo\cite{ref_almen1936}, which provides the below calculation for the washer's reaction force per deflection.
		...
		\label{app:calcs:cal:sat}
			...
			As discussed in Section~\ref{sec:sat:TF:results}, this calibration stack will occasionally be necessary to correct static machine dimension errors,
			whose expected range falls a maximum of \todo{upper range} above and \todo{lower range} below the nominal value.
			...
			Thus, to properly account for all linear error possibilities, the calibration assembly must be able to adjust $D_{eff}$ by no less than \todo{larger range}.
	...
	\label{app:calcs:sup}
		...
		\label{app:calcs:sup:sec}
			...
			This consist of various individual projects across a wide range of disciplines, any of which may be impacted by the \device\ and many of which are classified.
