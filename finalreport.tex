...
\label{sec:Specs}
    ...
    \label{sec:TF}
        ...
        Because each bellows applies a spring force against the actuation motion, these values must be evaluated through a force balance via a system of equations, yielding the final lengths of each critical component.
        These final values are summed to obtain the position of each square hole. Taking the difference between these positions, we obtain the height $X$ of the square orifice, which is input to the final transfer function $\hat{G}_O$ to obtain $D_{effective}$.

        

        Let us begin with the upper row of sub-transfer functions. 
