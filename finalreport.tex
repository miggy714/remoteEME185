...
\label{chap:DFMEA}
	...
	\section{Design Failure Mode Effects Analysis}
	\label{sec:DFMEA:intro}

		The high performance requirements and extreme operating conditions of the \device\ require in-depth risk assessment via Design Failure Mode Effects Analysis (DFMEA). Possible failure modes are identified
...
\subsection{Orifice Channel Opening}
	...
	the stress exerted on that corner, the Factor of Safety (FS) can then be calculated using the yield strength of Invar. Note that testing design and assumptions are detailed in \ref{chap:test}
    ...
    ($S_y = \SI{276}{\mega\newton\per\meter\squared}$)}
		...
		\begin{tabular}{l S[table-format=1.2e+2] c r}
			\toprule
			{Analysis Method} & {VM (\si{N/m^2})} & {Comparison} & {FOS} \\
			\midrule
			\rowcolor{offblue!40}
...
\label{chap:test}
	...
	\label{sec:comp_proto}
		...
		\begin{figure}[H]
			...
			\caption{A close-up of the orifice showing the pressure drop with isobaric surfaces}
					...
					\begin{enumerate}
						...
						\item Verification for the correctness of contact constraints is made
							...
							\begin{figure}[H]
								...
								\caption{Dispacement simulation}
