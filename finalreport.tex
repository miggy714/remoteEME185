...
\section{Risk Assesment and Analysis} % to separate the above subsections from those below
...
\begin{figure}[h]
                \centering
                \includegraphics[width=.75\textwidth]{ DFMEA_table_priority_modes.png}
	...
	Of the critical failure modes, two of them are on welds. \todo{finish weld calculations, insert calcs in Chapter~\ref{chap:comp}}
...
\begin{enumerate}[label=\roman*.]
	\item \textbf{Purpose.}
	The purpose of this analysis is to verify that the valve orifice maintains structural integrity with the fluid force that will be exerted on it. The orifice is a very small component of the system and also ciritical
	to the function of the device, so it is important to make sure that it will not fail under the pressure exerted on it by the internal gas flow.
	Here is image showing how the stress is concentrated at the edges of the orifice, which is where the failure is most likely to occur.
		\begin{figure}[h]
                \centering
                \includegraphics[width=.75\textwidth]{figures3/Orifice_edges_stress.png}
                \caption{Image showing the stress concentration at the edges of the orifice.}
                \label{fig:FEAdisp}
            \end{figure}
	\item \textbf{Assumptions, Principles, and Methods.}
	The assumptions for the valve orifice are using the computer software which calculates the stress on the surface of the orifice and finds the maximum and average von Mises stress. 
	The principles and methods for this analysis are the same as the ones used for the orifice channel opening, which are detailed in the following chapter, Testing.
	\item \textbf{Calculations.}
	Using the data collected from the computer software testing, the factor of safety can be found using von Mises stress and the yield strength of Invar. The average von Mises stress at the
	edges of the orifice was found to be 126 MPa and the maximum von Mises stress was found to be 362 MPa.
	\item \textbf{Results.}
	Using the maximum von Mises stress: 
	FOS = 240 MPa / 362 MPa = 0.66

	Using the average von Mises stress:
	FOS = 240 MPa / 126 MPa = 1.90

	The FOS less than 1 at the maximum stress is an issue and something that needs to
	be addressed, however because the average stress is greater than 1, it is likely that the
	corner will not fail under most operating conditions.
	\item \textbf{Discussion.}
	The FOS of less than 1 at the maximum von Mises stress indicates that the corners of the orifice could fail if the test got to the maximum stress, however because the FOS at the average von Mises stress 
	is greater than 1, it is likely that the corners will not fail under most operating conditions. One way to increase the FOS at the corners would be to round the corners that are not the ones creating the 
	diamond shape of the orifice. This would reduce the risk of failure at those corners and increase the FOS. 
\end{enumerate}

	\subsection{Tolerance Failure Analysis}
	\label{subsec:DFMEA::tol}
		...
		\begin{enumerate}[label=\roman*.]
			\item \textbf{Purpose.}
				...
				This provides a comprehensive estimation of the system's ability to meet the Required Relationship across its full temperature range.
			\item \textbf{Assumptions, Principles, and Methods.}
				...
				Other unquantifiable errors will be present that cannot be properly accounted for without construction of a physical prototype; these will be addressed in Chapter~\ref{chap:concl}.
			\item \textbf{Calculations.}
				...
				The function evaluates the adjusted temperature of each part, varies the machined length of all critical dimensions according to a normalized tolerance distribution, computes the thermal expansion of each part,
			...
			\item \textbf{Results.}
				...
				produces a normalized distribution. It estimates that 90.6\% of all devices machined will produce an output within tolerance across the full temperature range.
...
\label{chap:concl}
	...
	\begin{enumerate}
		...
		\item The mount would be better attached to the housing using fasteners due to the difficulty of welding dissimilar metals.
	...
	\todo{Nice short wrap-up paragraph}
