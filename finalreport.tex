...
\label{chap:Fintro}
	...
	\label{sec:intro:specs}
		...
		The Required Relationship, Mechanical Envelope, and certain operational conditions motivated the following design specifications.
		...
		\label{subsec:intro:specs:ME}
			...
			The subsystem mechanical-envelope specifications are given in lateral (X), vertical (Y), an longitudinal (Z) dimensions, respectively.
		...
		\label{subsec:intro:spec:final}
			...
			The final design architecture was strictly screened to meet these specifications to ensure that the \device\ exhibited a passive, linear, precise output with simple, compact, high-strength components.
...
\label{chap:design}
	...
	\label{chap:design:overview}
		...
		The \device\ contains and seals the high-pressure Hydrogen gas and delivers precise thermomechanical actuation with proven, aerospace-grade technology.
		...
		\label{subsec:design:overview:valve}
			...
			Figure \ref{fig:cut_zooms} shows the orifice in full assembly. Observe the moving centerpiece pressed against the outlet face the the housing inlet.
	...
	\label{sec:design:just}
		...
		\label{subsec:design:just:P}
			...
			The housing consists of the inlet, the outlet, and two center pieces, all of which are constructed from Invar 36 and analyzed for the device's ability to withstand the internal pressure. 
			The adjoined housing components provide the pressure boundary for the valve. The inlet and outlet sections have cylindrical internal geometries with threaded ends that connect to the pipe. The center of the valve sub-assembly (where the housing components meet) creates a void in the form of a rectangular prism that contains the orifice plate and bellows assembly.
			
			In order to determine the appropriate wall thickness required to sustain the internal pressure in the inlet and outlet sections, the housing is modeled as a thick-walled cylinder.
	...
	\begin{equation}
	\sigma_{vm} = \sqrt{\frac{(150.54 - (-68.95))^2 + (-68.95 - 40.80)^2 + (40.80 - 150.54)^2}{2}}
	...
	\begin{equation}
	FS = \frac{S_y}{\sigma_{vm}} = \frac{276}{190.08} \approx 1.45
	\label{fs}
	\end{equation}

				The wall thickness of \SI{7.806}{\milli\meter} produces a Von Mises stress of \SI{190.08}{\mega\pascal}, yielding a factor of safety of $FS \approx 1.45$.
				The housing inlet/outlet design ensures that \SI{7.806}{\milli\meter} is the shortest distance from the inner wall to the outer wall therefore the thickness at some points of the housing is greater than \SI{7.806}{\milli\meter}.
				Because the housing design is rectangular, not cylindrical, the calculation above is a conservative estimate of the stress, as realistic stress concentrations at the corners of the housing will be mitigated by the increased thickness at those points.
				
				Note that inlet and outlet wall thickness defines the lateral mechanical envelope of the valve. This design choice was driven by the need to maintain a pressure boundary while minimizing the overall size of the valve. Any increase in the wall thickness would directly increase the factor of safety, but also increase the overall lateral size.

				The housing center pieces are also designed with a wall thickness of \SI{10}{\milli\meter}, which is greater than the minimum required thickness of \SI{7.806}{\milli\meter}, to ensure that the pressure boundary is maintained at the critical junction where the three sections of the housing meet.
				...
				Stress concentrations at the corners of the prism were a primary concern and were modeled using COMSOL to verify this specific region did not experience excessive stress
				...
				\todo{Add comsol results for rectangular center section}       
		...
		\label{subsec:design:just:gen_dim}
			...
			\label{subsubsec:design:just:gen_dim:machine}
				...
				\todo{Any features we changesd for machinability. Delete if we don't have anything to say here. Maybe change order in report so that welding is introduced before this.}

			The \device\ is designed to be manufactured using mostly conventional machining techniques, such as milling and turning, with the critical welds performed using Electron Beam Welding (EBW) and the orifice apertures cut using Electrical Discharge Machining (EDM).
        One of the primary concerns during the design process was ensuring that the device could be assembled given the constraints of the manufacturing processes. The entire manufacturing and assembly process is detailed in Appendix \ref{app:mfg}, but the following design features were motivated by manufacturability:
        \begin{itemize}
            \item The housing center is split into two pieces to allow for line-of-sight welding of the bellows and valve orifice.
            \todo{add more as needed}
        \end{itemize}

        Due to the small scale of the device, there is little room to accommodate the welding process. For each welding operation, the weld joint must be continuously accessible by the welding beam, requiring an unobstructed line of sight throughout the full rotation of the workpiece.
        
        A key breakthrough in the design was the decision to split the housing center into two discrete pieces. In a single-piece housing configuration, the internal geometry of the housing would obstruct both the welding beam and any assembly tooling, making it impossible to reliably weld the bellows or seat the valve components without risking damage to adjacent parts. By separating the center into two pieces, each sub-assembly can be accessed and worked on independently before final integration.
        
        This modular approach enables the critical bellows welds to be performed outside of the housing entirely, with a full 360$^\circ$ line of sight, before the center pieces are joined to the inlet. Similarly, internal components that would otherwise be unreachable in a fully enclosed housing can be installed, inspected, and verified at each intermediate stage of assembly. The final closure welds, which join the two center pieces together and seal the assembly, are the last operations performed, ensuring that all internal work is completed and confirmed before the device is permanently closed. 
		...
		\label{subsec:design:just:cal}
			...
			See Appendix \ref{app:calcs:cal:force} for further calculations.
		...
		\subsection{Electron-Beam Welded Assembly}
		\label{subsec:design:just:LBW}
			...
			The device is assembled using Electron Beam Welding (EBW), a high-precision welding technique that uses a focused beam of electrons to join materials together\cite{ref_steen2010laser}.
			The \device requires three different types of welds:
			\begin{itemize}
				\item Welds joining bellows to the housing inlet and the valve orifice.
				\item Welds joining the housing center pieces to the housing inlet.
				\item A circumferential weld joining the housing inlet and center pieces to the housing outlet.
				\item A circumferential weld joing the mount to the housing.
				\item A weld joining the actuator to the valve stem.
			\end{itemize}

			Of these processes, the welds that joing the housing inlet, outlet, and center pieces are the most critical, as they form the pressure boundary of the valve and are subject to the highest stresses.
			The geomtry of the welds is a square butt joint, which is a widely used type of weld joint in pressure vessel construction allowing for full penetration of EBW.
	
			Full penetration is considered achieved when the weld fuses through the entire wall thickness $t$.
			EBW's high power density enables complete fusion across the interface, and the square butt profile 
			is the optimal geometry for radiographic interpretation, as it ensures defects cannot be obscured\cite{ref_steen2010laser}.
			Weld penetration quality is verified using 100\% X-ray radiographic inspection per ASME B31.3 
			Table 302.3.4\cite{ref_asme_b313}.
			The circumferential welds are certified to be free of defects such as porosity, lack of fusion, 
			and cracking, all of which could significantly reduce joint strength\cite{ref_steen2010laser}.
			\\~\\
			The viability of the weld joint is evaluated using the ASME B31.3 Pressure Piping Code 
			(Equation 3a or Equation \eqref{tmin_weld}, below)\cite{ref_asme_b313}, which governs the required minimum wall thickness as a function 
			of the weld joint quality factor $E$. Note that the calculation below are using a cylindrocal pipe geometry as a conservative approximation of the rectangular housing geometry, as the welds are located at the inner wall of the housing where the stress is highest and the wall thickness is lowest.
		...
		\begin{table}[H]
			...
			\begin{tabularx}{\textwidth}{l X l}
				...
				\\\addlinespace
				$Y$ &
				Coefficient for nickel alloys ($T < \SI{482}{\degreeCelsius}$) &
				0.4
				\\\addlinespace
				$E$ &
		...
		code factor of $S = S_y / 1.5 = \SI{276}{\mega\pascal} / 1.5 = \SI{184}{\mega\pascal}$.
		Per ASME B31.3 Table 302.3.4\cite{ref_asme_b313}, the weld joint quality factor $E$ depends entirely 
		...
		\begin{equation}
		t_{min} = \frac{\SI{68.95}{\mega\pascal} \times \SI{40}{\milli\meter}}{2\left(\SI{184}{\mega\pascal} \times 1.00 \times 1.0 + \SI{68.95}{\mega\pascal} \times 0.4\right)} 
		\approx \SI{6.52}{\milli\meter}
		\label{t_num}
		\end{equation}

		Since $t_{min} = \SI{6.52}{\milli\meter} < t_{actual} = \SI{7.806}{\milli\meter}$, the design is verified.
		The thickness safety factor at the weld joint is:

		\begin{equation}
		SF_{weld} = \frac{t_{actual}}{t_{min}} = \frac{\SI{7.806}{\milli\meter}}{\SI{6.52}{\milli\meter}} \approx 1.20
		\label{sf_weld}
		\end{equation}
		\\~\\
		As a secondary check, the peak hoop stress at the inner wall from the thick-wall analysis 
		($\sigma_\theta = \SI{150.54}{\mega\pascal}$) is compared directly against the weld joint allowable stress\cite{ref_asme_b313}:

		\begin{equation}
		\sigma_{allow} = S \cdot E \cdot W = \SI{184}{\mega\pascal} \times 1.00 \times 1.0 = \SI{184}{\mega\pascal}
		\label{stress_allow}
		\end{equation}

		\begin{equation}
		\sigma_\theta = \SI{150.54}{\mega\pascal} < \sigma_{allow} = \SI{184}{\mega\pascal}
		\label{ineq_hoop}
		\end{equation}

		The weld joint stress safety factor is:
		\begin{equation}
		SF_{stress} = \frac{\sigma_{allow}}{\sigma_\theta} = \frac{\SI{184}{\mega\pascal}}{\SI{150.54}{\mega\pascal}} \approx 1.22
		\label{sf_stress}
		\end{equation}

		These results confirm that with 100\% X-ray radiographic inspection ($E = 1.00$)\cite{ref_asme_b313}, 
		the weld joint introduces no strength penalty relative to the base material.
		The joint is viable with a thickness margin of \SI{1.29}{\milli\meter} and a stress safety factor of $1.22$.     
		...
		\label{subsec:design:just:mat}
			...
			\label{subsubsec:design:just:mat:Inv}
				...
				Invar, like ALLVAR, resists thermal conductivity $(k = 10.5)$. Heat transfer between the heated gas and the actuator subsystem is better inhibited than it would be by other metals.
...
\label{chap:sat}
	...
	\label{sec:sat:TF}
		...
		\todo{Uncomment pdf}
		\newpage
		%\includepdf[pages=1, fitpaper=true, landscape=true]{TF_import.pdf}
    ...
    \label{subsec:sat:TF:orif}
		...
		\label{sec:sat:TF:results}
			...
			are stochastic; their values must be iterated to find the worst-case combination.
			This combination is unlikely, however, and a uniform-distribution Monte Carlo analysis\cite{ref_figliola} yields a more informative expectation value from the propagated error.
			
			These stochastic combinations across the full temperature range produce the range shown in Figure \ref{TF_res},
			whose normalized distribution is plotted in Figure \ref{fig:error_dist}.
			
			Iterating through $\Tcold \leq T_{amb} \leq \Thot$ and plotting the results yields Figure \ref{fig:TF_res}.
			Two worst-case dimensional error combinations were found, one yielding too large an orifice across all temperature, and one yielding too small an orifice.
			These errors cannot be accounted for in the design; they differ between devices. However, once manufactured, these values remain constant.
			The calibration stack, discussed section \ref{subsec:design:just:cal} and further calculated in Appendix \ref{app:calcs:cal:sat}, can reliably correct these linear errors, shifting the whole plot linearly to within its specified tolerance.
			\todo{List avg, std dev}
...
\label{chap:concl}
	...
	\label{sec:conc:fin_anal}
		...
		\begin{itemize}
			...
			\item TF error assumes knowledge of all decimal points in dimensions (K only?)
			\item Nomenclature is weak. e.g. "Valve Orifice" and "Actuator" are terrible part names
			\item Only 50 micro meters belleville range on upper side; omits very small percentage of possible prototypes based on unlikely error assumption
		\end{itemize}
...
\begin{thebibliography}{99}
	...
	5th~ed., Wiley, Hoboken, NJ, 2011, Sec.~4.9.You said: Can you use the double-S symbol for sec?
	...
	\url{https://doi.org/10.3758/BF03206714}
	...
	\url{https://doi.org/10.3389/fnhum.2015.00198}
	...
	\bibitem{haas_vf2_specs} % HAAS tolerance for drawings
...
\label{app:calcs}
	...
	\label{app:calcs:cal}
		...
		\label{app:calcs:cal:sat}
			...
			As discussed in section \ref{sec:sat:TF:results}, this calibration stack will likely be necessary to correct static machine dimension errors, whose expected range falls a maximum of \SI{54.5}{\micro\meter} above and \SI{69.3}{\micro\meter} below the nominal value
			(429\% and 546\%, respectively, times the narrowest tolerance window.)

			Thus, to properly account for all linear error possibilities, the calibration assembly must be able to adjust $D_{eff}$ by no less than \SI{69.3}{\micro\meter}.
			...
			\begin{equation}
				D_{eff}(T_{ref}) = 292.1 \pm \SI{12.7}{\micro\meter} \rightarrow 2 \cdot \SI{12.7}{\micro\meter} = \SI{25.4}{\micro\meter} \text{ total resolution}
			\end{equation}

			Calibration with a resolution no more than \SI{25.4}{\micro\meter} precision will theoretically be able to achieve some setpoint withing the tolerable range.
			...
			Calibration adjustment directly translates to changes in the orifice height $X$. We will assume that the orifice height-diameter relation derived in Appendix \ref{app:calcs:orifice_geometry} hold true,
			as fillet-radius and horizontal-position errors do not conistently affect the diameter across the full temperature range and thus cannot be reliably eliminated by calibration:
			...
			Calibration is performed with the M2 screw 90362A141 from McMaster (specification drawing givin in Appendix \ref{app:datasheets:McMaster}),
...
\label{app:BOM}
	...
	\newpage
	\todo{Uncomment below PDFs: BOM, Invar quote, ALLVAR quote, Bellows quote, EDM quote}
	% \includepdf[pages=1, fitpaper=true, landscape=true]{BOM.pdf}

	\newpage
	% \includepdf[pages=?, fitpaper=true, landscape=true]{quote_Invar.pdf}
	
	\newpage
	% \includepdf[pages=?, fitpaper=true, landscape=true]{quote_ALLVAR.pdf}
	
	\newpage
	% \includepdf[pages=1-2, fitpaper=true, landscape=true]{quote_bellows.pdf}
	
	\newpage
	% \includepdf[pages=1-2, fitpaper=true, landscape=true]{quote_EDM.pdf}
...
\renewcommand{\thesection}{\arabic{section}}
	...
	\color{red}
		\begin{itemize}
			...
			\item Cite White 9th ed, eq. 6-102 on measuring effective diam. from pressure drop
			\item Invar billet: cut valve orifice out of same stock. Long dimension is along z axis; is at least 79 mm: 32 for Inlet, 40 for Outlet, 7 for thickness of valve orifice
		\end{itemize}
	\color{black}
...
\addtocontents{toc}{\protect\setcounter{tocdepth}{-1}}  % ← ADD THIS
\setcounter{section}{0}
\renewcommand{\thesection}{\arabic{section}}
...
\label{app:drawings}
	...
	The following appendix presents the detailed engineering and manufacturing drawings for the PTCD assembly and its individual components. General drawing tolerances govern the design requirements to ensure
	 reliability and precision.

	\includepdf[pages=1-12, fitpaper=true, landscape=true]{05182026_DrawingPackage_v31.pdf}
...
\todo{Update and reprint Gantt chart on the day of submission. Note that task list isnt required.}
