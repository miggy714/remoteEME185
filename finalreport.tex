...
\newcommand{\specActuatorCTE}{\SI{22.85e-6}{\per \celsius}} % 10^-6/ºC
...
\label{sec:Specs}
    ...
    \label{sec:TF}
        ...
        \todo{The below is very long and thin. Compact by making the bullet-point list two columns and putting figure \ref{fig:positional_block_diagram} on the left with text beside it.}
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[width=.5\textwidth]{positional_block_diagram_zoom.png}
    ...
    \end{multicols}
        ...
        Let us begin with the upper row of sub-transfer functions. Each part thermally expands, most of all the actuator. Each of these functions, then, inputs a given temperature and converts it to the thermally expanded dimension of the part $\ell$.
        These values are evaluated by the Force Balance and converted into their final, deformed length $L$.
        They are summed 

        For each of the above, X variables must be defined:

        \begin{itemize}
            \item $\hat{G}_i$ = thermal expansion transfer function; inputs temperature, outputs the part's expanded dimension
        \end{itemize}
