...
\todo{Set page number counter to start at 2 next page}
...
stipulates that the orifice maintain an effective diameter that varies with temperature between 170 and \SI{380}{\micro\meter}, or half the width of the ballpoint on a pen,
with an average tolerance of $\pm \SI{13}{\micro\meter}$, about the thickness of a human hair.
...
\label{chap:intro}
	...
	\label{sec:intro:motive}
		...
		\label{subsec:intro:motive:background}
			...
			Passive temperature-compensating devices exist on the current market, \todo{maybe mention that most have a binary function and do not allow for flow adjustment} but none approach the size or performance characteristics needed.
	...
	Our three givens --- the Required Relationship, Mechanical Envelope, and operational conditions --- drive the design specifications, which will in turn motivate the optimal design architecture.
		...
		\label{subsec:intro:spec:final}
			...
			\begin{table}[H]
				...
				V.1.1 & \SpecVAA &
				\multirow{3}{=}{\vspace{10em}A.1.1} & \multirow{3}{=}{\SpecAA} \\
...
\label{chap:design}
	...
	\label{chap:design:overview}
		...
		\label{subsec:design:overview:act}
			...
			Even this deviation is significant relative to the 10-\SI{20}{\micro\meter} tolerance window for the effective diameter.
	...
	\label{sec:design:just}
		...
		\label{subsec:design:just:TF}
			...
			\label{subsubsec:design:just:TF:valve}
				...
				below the top surface of the housing, including a 5 mm gap between that and the valve stem-actuator interface at this temperature.
			...
			\end{align*}
				...
				This bimetallic architecture enables the actuator to be half the length of solitary high-expansion metal, keeping its total length (45.55 mm, accounting for the mount's upper wall and the screw that protrudes from its top) within the specified vertical mechanical envelope.
		...
		\label{subsec:design:just:P}
			...
			Principal Stresses:
			\begin{equation}
			\sigma_{\theta} = P \left( \frac{r_o^2 + r_i^2}{r_o^2 - r_i^2} \right) = 68.95 \left( \frac{19.53^2 + 9.53^2}{19.53^2 - 9.53^2} \right) \approx \SI{112.05}{\mega\pascal} \quad (\text{Hoop Stress})
			\label{hoop}    
			\end{equation}
		...
		\end{equation}
			...
			\begin{equation}
			\sigma_{z} = P \left( \frac{r_i^2}{r_o^2 - r_i^2} \right) = 68.95 \left( \frac{9.53^2}{19.53^2 - 9.53^2} \right) \approx \SI{21.55}{\mega\pascal} \quad (\text{Longitudinal Stress})
			\label{longitudinal}
			\end{equation}
			\\~\\
			...
			code factor of $S = S_y / 1.5 = 370 / 1.5 = \SI{246.7}{\mega\pascal}$.
			...
			\begin{equation}
			\sigma_{allow} = S \cdot E \cdot W = 246.7 \times 1.00 \times 1.0 = \SI{246.7}{\mega\pascal}
			\label{stress_allow}
			\end{equation}
		...
		\label{subsec:design:just:gen_dim}
			...
			\label{subsubsec:design:just:gen_dim:act_diam}
				...
				The deflection decrease reaches a point of diminishing return near a total outer diameter (OD) of 25 mm (Figure \ref{fig:mount_OD_opt}).
				...
				The total deflection is minimized at a mount wall thickness of 3.5 mm (Figure \ref{fig:mount_thick_opt}).
				...
				\begin{figure}[H]
					...
					\begin{tikzpicture}
						...
						\legend{Cold excursion ($T_\text{low}=-60^\circ$C),
										Hot excursion ($T_\text{high}=80^\circ$C)}
				...
				\begin{figure}[H]
					...
					\begin{tikzpicture}
						\begin{axis}[
								...
								ylabel={$\delta_{\text{elastic,max}}$ ($\upmu$m)},
								xmin=0, xmax=12,
								ymin=0, ymax=1.0,
						...
						\addlegendentry{Series min @ $t_{\text{wall}} = 3.5$ mm}
			...
			\label{subsubsec:design:just:gen_dim:thread}
				...
				As detailed by Sandia, gas will be supplied by a stainless steel pipe between 1.524 mm and 25.4 mm in diameter.
				This design features a 19.05 mm (3/4") threaded interface, which diameter can be adjusted in further design iterations as needed for particular experiments.

				The housing inlet and outlet provide 15 and 20 mm threaded length for an airtight seal.
				Adding the space needed for the tapered drill-bit cut and the walls that support the valve orifice, the valve subsystem's longitudinal mechanical envelope totals 72 mm, well within its design specification.
			...
			\label{subsubsec:design:just:gen_dim:ME}
				...
				As stated in various sections above, each subsystem met its mechanical envelope specifications, with one exception: the valve subsystem violates its vertical mechanical envelope, pushing the total system height to $(45.55 + 70.72) = \SI{116.27}{\milli\meter}$, a 16\% violation of the required mechanical envelope.
		...
		\label{subsec:design:just:cal}
			...
			The calibration stack, pictured in Figure \ref{fig:cal}, allows the user to set the output of the device at a single temperature, shifting the whole system output closer to the center of its tolerance.
		...
		\label{subsec:design:just:mat}
			...
			The Ashby Chart\todo{cite} (Figure \ref{fig:ashby}), an engineering standard for thermal stress of materials, provides a fitting guide for the relative strength and thermal expansivity of various metals, polymers, and ceramics.
			...
			\label{subsubsec:design:just:mat:Al}
				...
				Aluminum\cite{ref_NASA_specs_Al} stands alone as a strong, expansive, consistent metal. It has a CTE of \specActuatorCTE\ at $T_{ref}$ that varies a maximum of \SI{1}{\micro\meter\per\meter\per\celsius} as $\SI{-60}{\celsius} \leq T_{amb} \leq \SI{80}{\celsius}$.
			...
			\todo{All other nontrivial dimensions, design choices, and other issues go in this section (\ref{sec:design:just}).}
...
\label{chap:sat}
	...
	\label{sec:sat:error}
		...
		\label{subsec:sat:error:CTE}
			...
			$Error = \Delta L_{actual} - \Delta L_{expected} = 0.1363 \text{ mm} - 0.1339 \text{ mm} = \SI{2.4}{\micro\meter}$
			...
			Using the dimensional tolerance of $\SI{5}{\micro\meter}$ for $L_0$, we can combine these values as indicated in Table \ref{tab:act_specs_disc}:

			$$\sqrt{ \left( L_0 u_{\alpha} \right) ^2 + \left( \alpha u_{L_0} \right) ^2} = \sqrt{ \left( 84.335 * \SI{2.4}{\micro\meter} \right) ^2 + \left( 23E-6 * \SI{5}{\micro\meter} \right) ^2} = \SI{0.2}{\micro\meter} \leq \SI{0.3}{\micro\meter}$$
...
\todo{Rearrange citations so appear in order of appearance; test all links}
...
\label{app:calcs}
	...
	\label{app:calcs:belle}
		...
		\textbf{Belleville washer (part 93237A101) specifications}\cite{ref_McMaster_belle, ref_ledbetter1981}
...
\renewcommand{\thesection}{\arabic{section}}
	...
	\color{red}
		\begin{itemize}
			\item Do we need to heat-treat the Invar parts?
...
\label{app:datasheets}
    ...
    \begin{table}[H]
        ...
        \toprule
        \textbf{Material} &
        \textbf{CTE} $\left(\frac{\upmu\text{m}}{\text{m}\cdot\degC}\right)$ &
        \textbf{$E$} (GPa) &
        \textbf{$\nu$} \\
        \midrule
...
\end{figure}    
	...
	Our team, the Noble Aggie Labs, extends our deepest gratitude to Prof. Jon Schofield of UC Davis, his teaching assistant Xiangpu Wang, and most especially Dr. Bon of Sandia National Laboratories for their invaluable assistance in the delopment of this project. 
