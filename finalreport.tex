...
\label{chap:Fintro}
	...
	\label{sec:intro:motive}
		...
		\label{subsec:intro:motive:background}
			...
			Sandia's Gas Transfer Systems team, based in Livermore, CA, develops specialized gas flow control components and assemblies that help design, certify, and calibrate any of the myriad projects undertaken at the Lab.
			...
			They would require a fully passive thermomechanical valve, one that could control the flow of high-pressure gas across a wide temperature range with high precision.
			...
			Therefore, despite this design foundation, a passive, precise valve that continuously manipulates fluid flow without responding to the fluid's conditions would prove a novel and challenging exercise.

		\subsection{Objectives}
		\label{subsec:intro:motive:objectives}

			Thus it is that Sandia National Labs (SNL, aka the ``sponsor'') employed the Noble Aggie Labs, a mechanical design team at the University of California, Davis and the authors of this paper,
			to develop a high-pressure, hydrogen-compatible, Passive Temperature Compensation Device (\device)\cite{ref_sandiaprompt} that precisely controls a submillimeter orifice in response to a wide ambient temperature range
			...
			\begin{itemize}
				...
				\item Adjust the orifice size in response to the ambient external temperature, without regard to internal flow conditions, with an effective-diameter tolerance of up to \SI{20}{\micro\meter}\footnote{No more than the width of a human hair\cite{ref_NIH}.}.
	...
	\label{sec:intro:specs}
		...
		\begin{itemize}
			...
			\item An actuator to respond to temperature changes and drive the valve's movement
		\end{itemize}
		...
		this will remain quantified at a system level.
			...
			\begin{multicols}{2}
...

				{
					...
					\begin{align*}
						...
						D_{ref} &\text{ is set by the valve's initial position.}
					\end{align*}
				}
...
\label{chap:DFMEA}
	...
	\section{Design Failure Modes and Effects Analysis}
	\label{sec:DFMEA:intro}

		The high performance requirements and extreme operating conditions of the \device\ require in-depth risk assessment via Design Failure Modes and Effects Analysis (DFMEA). Possible failure modes are identified
		and ranked by Risk Priority Number (RPN) according to their impact on the \device's satisfaction of its functional and performance specifications. RPNs are calculated for each failure mode
		by taking the product of the qualitative ranking of individual factors of failure severity, probability of occurrence, and ease of detection.
		Not all failure modes could be identified in the time available, so those deemed most critical (having an RPN $> 100$) will be analyzed here;
		they are listed in Table~\ref{tab:RPN_priority}.
		The rest are given in Appendix~\ref{app:iter:DFMEA} for reference during further iteration.
		
		\vspace{1em}
		\begin{table}[h]
				\centering
				\caption{Prioritized failure modes with RPN $>$ 100}
				\label{tab:RPN_priority}
				\footnotesize
				\begin{tabularx}{\textwidth}{>{\centering\arraybackslash}p{1.0cm} X X X X X}
						\toprule
						\textbf{Comp.\ \#} & \textbf{Subsystem / Component Description} & \textbf{Component Function} & \textbf{Potential Failure Mode} & \textbf{Potential Effect of Failure} & \textbf{Potential Cause of Failure} \\
						\midrule
						2b & Housing Outlet Thin Wall & Supports valve orifice & Deformation & Loss of function, inaccurate test results & Pressure differential forces orifice into wall \\[4pt]
						2c & Housing Outlet Channel & Restricted diam channel from orifice & Deformation & Decreased function, small damage & Large pressure differential \\[4pt]
						2d & Housing Center Weld & Holds valve assembly & Stress failure & Damage to internals, loss of function & Stress concentrations \\[4pt]
						4  & Actuator Radial Weld & Fixes and seals actuator & Stress failure & Decreased function, inaccurate test, small damage & Thermal cycling, induced stress at weld toe \\[4pt]
						10 & PTCD System & Entire Device Assembly & Outside of error tolerance & Inadequate accuracy & Accumulated inaccuracy \\
						\bottomrule
				\end{tabularx}
		\end{table}
		\todo{Clean this up a little}

		Failure is defined as the inability of the device to meet its specifications.
		For the \device, this includes plastic\footnote{i.e. Irreparable.} deformation, fracture, rupture, or simple exceedance of the system output tolerance.
		Each failure mode identified must undergo analytical and computational testing to confirm reliability and performance of the current PTCD design
		or to inform design modifications.

		Each of the below analyses documents the failure mode, the assumptions, principles, and methods applied, the calculations performed, and the results of the analysis.

	\section{Weld Failure Analysis}
	\label{sec:DFMEA:weld}
		\subsection{Housing Center Weld}
		\label{subsec:DFMEA:weld:main}

			The pressure capacity of the housing is verified in Section~\ref{subsec:design:just:P}. The circumferential weld between the Housing Inlet and Outlet can be reliably assumed to
			fuse the housing material together without sacrificing yield strength, but this assumption must be further evaluated to ensure the weld can withstand the significant axial forces applied by the internal pressure.
			...
			Equation~\eqref{eq:stress_bending} shows that bending stress is minimized by reducing 
			...
			To better model the bending failure of the housing wall, the housing structure was simulated using a finite-element mesh.
			The resulting stresses can be found in Table~\ref{tab:FEAdisp2}, presented later.
			...
			Equation~\eqref{eq:stress_bending} confirms that bending is controlled by the 
		...
		\subsection{Actuator-Housing Weldment}

		In addition to stresses applied by the internal pressure, there is also stress on the 
    ...
    The weld joints are treated as full penetration fillet welds in this analysis, and the same assumptions made 
		for the housing weld analysis hold for the material properties and radiographic inspection 
		of the actuator-housing weld. The mount being made of ALLVAR has an anisotropic quality. It has a positive radial coefficient
  	of thermal expansion that is 2 $\mu$m/m greater than the magnitude of its negative axial, 
  	expansion amounting to $\alpha_{\text{ALLV-radial}}=32 \mu$m/m. The greatest change in radius to the mount $\Delta R$ occurs across the cold side of the 
  	temperature gradient using the maximum machined outer radius $R_0 = 9.0215 mm$. It is found that
		$\Delta R = R_0 \left(\alpha_{\text{ALLV-radial}}\right) \Delta T_{\text{cold}}=0.0254mm$
		...
		\begin{equation}
			\sigma_{\text{ALLV}} = \frac{E_{\text{ALLV}} \cdot \Delta R}{R_0} = \frac{55\times10^{9} \cdot 0.0254 \text{ mm}}{18 \text{ mm}} = 74.86 \text{ MPa}
		\end{equation}
		The area of the weld and the area of the mount in contact with it are the same, and thus the
 		stresses are the same. Given that this stress is much smaller than the yield stress of ALLVAR and Invar,
		 295 MPa and 276 MPa respectively, we find a sufficient FS. Using the more conservative yield stress of Invar the FS is calculated to be:
		$$\text{FS} = \frac{S_{y,\text{weld}}}{\sigma_{\text{weld}}}=3.68$$
		This FS is acceptable, but in reality, the material mismatch in the mount-housing weld may lead to 
 		detrimental changes in material properties around the weld area which could cause a variety
  	of failures. Because ALLVAR 30 is an experimental alloy with a proprietary composition we 
  	...
  	welding ALLVAR to Invar or other iron alloys as the titanium from the ALLVAR will combine with
   	the iron to create brittle intermetallic compounds. This in turn makes the weld brittle and more susceptible to cracking and stress failures. 
		...
		creation of brittle compounds, but this could be highly involved and experimental. A recommended
 		path is to use fasteners with a slight mount redesign that allows for bolting into the
  	housing. Note that this weld does not make contact with the fluid and thus does not need 
		to seal, and concerns regarding hydrogen compatibility can also be relaxed.

\section{Component Analysis}
The following are issues identified as critical for components of the PTCD. These components were deemed critical
because they facilitate the decreasing flow cross section and thus are exposed to large pressure differentials, 
or else contain geometry that can concentrate stresses. 
\subsection{Housing Outlet Wall}
	...
	hand calculations, the FS can be calculated to determine if the wall is thick enough to withstand the pressure exerted on it.
	...
	where P is the pressure, r is the radius of the plate, and t is the thickness of the plate.
        ...
        \text{FS} = \frac{S_{y,\,\text{Invar}}}{\sigma_{\max}}
                   = \frac{276~\text{MPa}}{158.4~\text{MPa}}
                   = 1.74
    \end{equation}
	...
	is not enough to significantly affect the effective diameter or anything else that would cause the device to fail. This shows that this failure mode will
	not be a major concern for the design and the wall is thick enough so that the pressure will not cause it to fail. 


\subsection{Orifice Channel Opening}
	...
	the stress exerted on that corner, the Factor of Safety (FS) can then be calculated using the yield strength of Invar. Note that testing design and assumptions are detailed in Chapter~\ref{chap:test}

	See Table~\ref{tab:FEAdisp2} for tabulated data from the FEA testing that will be used to calculate the FS at the corner of the outlet opening.

	\vspace{1em}
	\begin{table}[h]
		\centering
    \caption{von Mises stresses compared against Invar yield strength
    ($S_y = \SI{276}{\mega\newton\per\meter\squared}$)}
		\label{tab:FEAdisp2}
		\begin{tabular}{l S[table-format=1.2e+2] c r}
			\toprule
			{Analysis Method} & {VM (\si{N/m^2})} & {Comparison} & {FS} \\
			\midrule
			\rowcolor{offblue!40}
			Max Outlet Hole     & 2.66E+08 & $V_m < S_y$ & 1.04 \\
			\rowcolor{ucdgold!50}
			Avg Outlet Hole      & 7.52E+07 & $V_m < S_y$ & 3.67 \\
			\rowcolor{offblue!40}
			Max Outlet Cone     & 3.03E+08 & $V_m > S_y$ & 0.91 \\
			\rowcolor{ucdgold!50}
			Avg Orifice Channel  & 1.78E+08 & $V_m < S_y$ & 1.55 \\
			\rowcolor{offblue!40}
			Max Orifice Channel & 3.00E+08 & $V_m > S_y$ & 0.92 \\
			\rowcolor{ucdgold!50}
			Avg Outlet Cone      & 1.51E+08 & $V_m < S_y$ & 1.83 \\
			\rowcolor{offblue!40}
			Max Center Back     & 3.43E+08 & $V_m > S_y$ & 0.81 \\
			\rowcolor{ucdgold!50}
			Avg Center Back      & 1.34E+08 & $V_m < S_y$ & 2.05 \\
			\rowcolor{offblue!40}
			Max Center Side     & 2.91E+08 & $V_m > S_y$ & 0.95 \\
			\rowcolor{ucdgold!50}
			Avg Center Side      & 5.98E+07 & $V_m < S_y$ & 4.62 \\
			\rowcolor{offblue!40}
			Max Center Top      & 1.37E+08 & $V_m < S_y$ & 2.01 \\
			\rowcolor{ucdgold!50}
			Avg Center Top       & 7.52E+07 & $V_m < S_y$ & 3.67 \\
			\rowcolor{offblue!40}
			Total Maximum                & 4.91E+08 & $V_m > S_y$ & 0.56 \\
			\rowcolor{ucdgold!50}
			Total Average                 & 6.03E+07 & $V_m < S_y$ & 4.58 \\
			\bottomrule
		\end{tabular}
	\end{table}
	...
	The maximum von Mises stress at the corner of the outlet opening is 300 MPa and the average von Mises stress is 178 MPa. Using the yield strength of annealed Invar (276 MPa), 
	the FS can be calculated for both the maximum and average stress values.
	
	Using the maximum von Mises stress:
	$FS = \frac{276 \text{MPa}}{300 \text{MPa}} = 0.92$

	Using the average von Mises stress:
	$FS = \frac{276 \text{MPa}}{178 \text{MPa}} = 1.55$

	The FS less than 1 at the maximum stress is an issue and requires further analysis. It is, however, the maximum of a small concentration that is much higher than the stresses across the rest of the surface.
	This is confirmed by the FS for the average stress being acceptable. Further analysis will show whether the change in geometry creates an actual stress concentration, or if results are skewed by the FEA due to computational errors at this point.
	Intuition from these results suggests that if the corner is rounded to reduce the stress concentration it would increase the FS. Another way to increase the FS
	...
	\label{subsec:DFMEA::tol}
		...
		The final failure mode --- namely, failure to meet the Required Relationship in Equation~\eqref{eq:required_relationship} --- encapsulates several sources of error that affect the input-output relationship of the \device.
		...
		The open-loop passive system transfer function in Figure~\ref{fig:TF_RR} must be revised to account for six sources of error:
		heat transfer from the hot internal gas, dimensional errors, variations in the actuator's thermal expansion, thermal expansion of the valve subcomponents, the spring reaction force of the bellows, and the orifice fillet radius produced by the wire EDM cut.

		Derivation and calculation of the full System Output Transfer Function is presented in Appendix~\ref{app:sat}. The derived values were encoded in a script to enable efficient parameter variation.
		...
		specified in Appendix~\ref{app:calcs:cal:sat} (a \SI{14.3}{\degree} rotation of the calibration screw) is met or exceeded.
...
\chapter{Computational Prototype}
\label{chap:test}
	...
	\section{Objective}
		...
		Dynamics (CFD), and Multi-Body Dynamics (MBD).
		Analysis and simulation were performed using COMSOL Multiphysics\cite{ref_COMSOL}.

	\section{Test Design}

		To make the simulations less computationally expensive, the following
		strategies have been employed:

		\begin{enumerate}
			\item The PTCD is cut in half about its vertical-longitudinal symmetry plane, creating
				new surfaces which will be referred to as ``symmetry surfaces'' shown in Figure~\ref{fig:symmetryplane}. By setting
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=.25\textwidth]{testingch_symmetry.png}
				\caption{Symmetry plane}
				\label{fig:symmetryplane}
			\end{figure}

			\item The physics is ``decoupled''. The CFD is run on a rigid negative
				domain (seen in Figure~\ref{fig:fluidvolume}) of the channel with a turbulent simulation where a ``pressure map''
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=.25\textwidth]{testingch_negative.png}
				\caption{Fluid domain, conformed to cavity volume}
	...
	\label{sec:comp_proto}
				...
				\item The inlet and outlet are assumed to be in fully developed flow
					\begin{enumerate}
						\item Inlet pressure is swept across the design space with parameters
							from $10^{2}$ psi to $10^{4}$ psi in increments of $10^{0.5}$ psi.
		...
		wetted surface of the multi-body simulation. The pressure map is shown to be created by the fluid dynamics as shown in Figure~\ref{fig:pressuremap}.
		\begin{figure}[H]
			\centering
			\includegraphics[width=.75\textwidth]{testingch_pressuremap.png}
			\caption{Orifice pressure drop with isobaric surfaces}
		...
		\subsection{Heat Transfer Simulation}
			...
			by a steady state(Figure~\ref{fig:tempgrad}) and transient study.
			\begin{figure}[H]
				\centering
				\includegraphics[width=.45\textwidth]{testingch_tempgrad.png}
				\caption{Maximum temperature gradient}
				\label{fig:tempgrad}
			\end{figure}
		...
		\subsubsection{Boundary Condition Decisions}
			...
			For the steady state heat transfer test, the operating temperature range
			...
			$h$ and an external ambient temperature $T_{amb}$. The ambient temperature was
			driven by the following step function:
			\begin{equation}
					T_{amb}(t) = T_{ref} + \SI{20}{\degreeCelsius}\times \mathrm{tstep}(t),
			\end{equation}
		...
		\subsubsection{Data Collection}
			...
			the Results section. The sweep was run in \SI{5}{\degreeCelsius} increments with
			...
			time-dependent solve for the \SI{20}{\degreeCelsius} ambient step. The point
			temperature and corresponding displacement were tested at fixed output intervals over a
			two-hour simulated window $t \in [0, 7200] \si{\second}$ at \SI{60}{\second} intervals. This window was chosen
		...
		\subsubsection{Limitations}
			...
			entire PTCD assembly. This model included the actuator, the housing, cylindrical
			...
			contact resistance at the bonded interfaces was also omitted. This assumes the
			...
			By and large the simplifications in the model are at best valid characterizations
			of the differential-expansion stroke mechanism and its approximate transient
			behavior.

		\subsection{Multi-body Simulation}

			To establish an intuition on the PTCD housing's reaction to applied pressure
				...
				\item Surface pairs and contact constraints.
					...
					\begin{enumerate}
						\item In multi-body simulation, the consideration for surface pairs
							...
							as \textbf{contact pairs} as shown in Figure~\ref{fig:both_label}.
						\vspace{1\baselineskip}
							\begin{figure}[H]
								\centering
								\begin{subfigure}[t]{0.45\textwidth}
									\centering
									\includegraphics[width=\textwidth]{testingch_contact_pair.png}
									\caption{Demonstration of how surfaces set as contact pair react}
									\label{fig:contactpair}
								\end{subfigure}
								\begin{subfigure}[t]{0.45\textwidth}
									\centering
									\includegraphics[width=\textwidth]{testingch_identity_pair.png}
									\caption{Demonstration of how identity pairs simulate a weld}
						...
						\item Verification for the correctness of contact constraints is made
							by setting a rigid translation of the orifice in space and observing how
							the meshes behave. This is shown on Figure~\ref{fig:displacement} below.
							\begin{figure}[H]
								\centering
								\includegraphics[width=.90\textwidth]{AbrAkAdAbrA_teST.png}
								\caption{Displacement simulation}
								\label{fig:displacement}
							\end{figure}
					\end{enumerate}

				\item Fluid-solid boundaries (incorporating union nodes where a single
					continuous boundary is required for Fluid-Structure Interaction).
		...
		\subsubsection{Data Collection}
			...
			DFMEA. These data can be recovered at any point in the PTCD as shown in Figure~\ref{fig:stressnstrain}.

			\begin{figure}[H]
				\centering
				\begin{subfigure}[t]{0.45\textwidth}
					\centering
					\includegraphics[width=\textwidth]{testing_ch_volstrain.png}
					\caption{Volumetric strain}
					\label{fig:straindata}
				\end{subfigure}
				\begin{subfigure}[t]{0.45\textwidth}
					\centering
					\includegraphics[width=\textwidth]{testing_ch_volstress.png}
					\caption{von-Mises stress}
					\label{fig:stressdata}
				\end{subfigure}
				\caption{Stress-strain map}
	...
	\section{Other relevant results/Discussion}
		...
		\subsection{Heat Transfer}
			...
			The two FEA anchor points at $\Delta T = \SI{5}{\degreeCelsius}$ and
			$\Delta T = \SI{10}{\degreeCelsius}$ returned actuator displacements of
			\SI{5.58}{\micro\meter} and \SI{11.14}{\micro\meter}, establishing a slope of
			\SI{1.115}{\micro\meter\per\degreeCelsius} and matching the analytical prediction
			within \SI{0.2}{\percent}.

			The time-dependent study, run for a representative \SI{20}{\degreeCelsius} ambient
		...
		\subsection{Design space}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=.90\textwidth]{testingch_designspace.png}
			...
			orifice diameter at the maximum pressure of \specPressure at an ambient temperature
			of \SI{0}{\degreeCelsius}.
...
\label{app:calcs}
	...
	\label{app:calcs:cal}
		...
		\label{app:calcs:cal:sat}
			...
			whose thread pitch equals \SI{0.4}{\frac{\milli\meter}{thread}}. Thus, a \SI{360}{\degree} turn of the screw corresponds to a \SI{0.4}{\milli\meter} change in the orifice height, meaning

			\begin{equation}
				\left( \SI{12.7}{\micro\meter} \right) \sqrt{\frac{\pi}{2}} \frac{\SI{360}{\degree}}{\SI{400}{\micro\meter}} = \SI{14.3}{\degree}
			\end{equation}

			A \SI{14}{\degree} rotation is a very reasonable demand for a manual screwdriver\cite{ref_jones1992, ref_Cappello2015}. As discussed in Section~\ref{app:calcs:cal:force}, this can be performed by hand with little effort,
	...
	\label{app:calcs:sup}
		...
		\subsection{Additional Testing Data}
		\label{app:calcs:sup:test}

			The table below is a direct result from the steady-state test. It shows a 0.2\% agreement with the analytical results compared to the FEA results.
			...
			\begin{tabular}{rrr}
			\toprule
			$T_{env}$ (\si{\celsius}) & $\Delta T$ (\si{\degreeCelsius}) & Stroke (\si{\micro\meter}) \\
			\midrule
			$-60$ & $-85$ & $94.78$ \\
			$-50$ & $-75$ & $83.62$ \\
			$-40$ & $-65$ & $72.47$ \\
...
\label{app:iter}
	...
	\section{Comprehensive Failure Analysis}
	\label{app:iter:DFMEA}
		...
		of low-fidelity prototypes, whose lifetime is unimportant. As the device reaches later phases of testing, life cycle analysis will become necessary.

		\newpage
		\includepdf[pages=1, landscape=true, fitpaper=true]{DFMEA.pdf}
	...
	\label{app:iter:weak}
		...
		\begin{itemize}
			\item The Valve Orifice and Housing Outlet, which each contain one component of the diamond orifice, are pressed flush against one another by a precision fit.
				This flush contact is assumed to only permit flow through the hole cut into the Valve Orifice without allowing leakage around its sides, but this was never verified.
				The tolerance of the fit may need to be adjusted, or additional sealing applied to ensure gas is not allowed to escape into the rectangular cavity of the housing.
			\item The calibration screw may interfere with the expansion of the aluminum actuator due to a difference in expansion rates. This can be solved by shortening the screw, using an aluminum screw, or both.
				The calibration assembly is modeled as a simple fusion between the upper surface of the Actuator and the underside of the belleville washer, assuming no thermal expansion in the small space above this plane.
				This model is inaccurate and should be investigated more thoroughly. The calibration screw, belleville washer, and mount material above the actuator will all thermally expand a small amount.
			\item The strength of the Electron Beam welds used for assembly is assumed to be ideal, an assumption defended in Section~\ref{subsec:design:just:weld_overview}. Though EBW is the most precise weld available, the weld will likely
			...
			\item Measures can be taken to improve the thermal isolation and convection of the actuator subsystem. The housing material between the actuator and the internal components
				can be increased, insulating the actuator against conduction.
			...
			This can be taken further, cutting fins into the actuator structure (typically done with wire EDM as with the orifice holes) to maximize convection with the ambient air.
			...
			\item As detailed in Section~\ref{subsec:design:just:mat}, materials were carefully selected to maximize thermal stability, strength, and consistency.
				While ALLVAR, Invar, and Inconel can be confidently relied upon as the best choice for their application, aluminum may be better replaced with an alloy with a higher thermal expansion rate.
				Many magnesium and zinc alloys possess exceptional CTE properties, but those found exhibited unacceptable variation across the wide temperature range. In the time provided, however, not all
				alloy variations could be explored, so an option may exist with a stable CTE higher than that of aluminum, allowing for a more compact actuator.
			\item The lower bellows may not be necessary. As derived in Section~\ref{subsec:design:just:bellows}, it eliminates an unbalanced pressure force on the underside of the Valve Orifice, which translates into compression of the valve stem and actuator.
			\item Similarly, the bellows could be replaced with the I718-159-72-2300\cite{ref_miniflex_catalog}, which has a higher squirm and burst rating and \nicefrac{1}{3} the free length of the I718-159-72-430
				...
				As described in Section~\ref{app:iter:customize}, the effect of removing the lower bellows and replacing the upper bellows with a shorter, stiffer alternative
				can be readily analyzed using the Transfer Function developed during the verification process described in Chapter~\ref{chap:DFMEA},
				allowing for a more informed trade-off analysis that may greatly reduce the \device's mechanical envelope with minimal effect on the system output.
				To further mitigate compression due to internal forces, the Actuator subsystem diameter can be readily increased, proportionally increasing the system spring constant to the square of any diameter increase
				(see Equation~\eqref{eq:k}).
	...
	\label{app:iter:customize}
		...
		\label{app:iter:customize:TF}
			...
			\begin{itemize}
				...
				\item To introduce estimated friction or pressure forces, a constant load can be applied to the force balance in Section~\ref{app:sat:TF:force_balance} with little change to the derivation.
		...
		\label{app:iter:customize:RR}
			...
			\label{app:iter:customize:RR:slope}
				...
				The slope of system output, set to the constant $A_1$ in this design, is defined by the critical actuator dimensions as derived in Section~\ref{subsubsec:design:just:TF:act}.
...
\label{app:BOM}
	...
	The full Bill of Materials and accompanying vendor quotes are provided on the following pages, with corresponding part numbers, materials, sizes, prices, and distributor links.
	A quote for the Wire Electrical-Discharge Machining (EDM) operation required to cut each square hole is also included from Milco Wire EDM, Inc; achievable tolerance values given in Appendix~\ref{app:sat:error:mach} were taken from this quote.
	...
	\begin{figure}[H]
		...
		\caption{Valve subsystem part IDs}
	...
	\begin{figure}[H]
		...
		\caption{Actuator subsystem part IDs}
...
\chapter{Manufacturing and Operation}
	...
	\begin{notebox}
		...
		\begin{itemize}[itemsep=2pt]
			\item Computer-Numerical Control (CNC) mill and lathe that can meet or exceed the dimensional
	...
	Table~\ref{tab:mfg_parts} summarizes all parts manufactured or purchased for this device.
	...
	\begin{enumerate}[itemsep=4pt]
		...
		\item \textbf{Finish all faces} except for the -Y face, paying especial attention
					to the +X face.
		\item \textbf{Mill valve stem}. From several directions perpendicular to the vertical axis,
	...
	\begin{enumerate}[itemsep=4pt]
		...
		\item \textbf{Mill to length:} Face both $+$Y and $-$Y ends with a 12 mm end mill to bring the part close to nominal length.
		...
		\item \textbf{Drill central hole:} Drill the axial hole
					through the top of the part with a 1.6 mm drill bit, centered on the axis.
					\newpage
					\begin{notebox}
	...
	\begin{enumerate}[itemsep=4pt]
		...
		\item Invert the housing-actuator assembly. Carefully insert the actuator into
	...
	The PTCD is designed for use in high-pressure gas experiments in a controlled laboratory setting.
	The device is intended to regulate flow of noble gases and hydrogen using passive thermal expansion given ambient temperature differences,
	and is rated for pressures up to \SI{68.95}{\mega\pascal} (\num{10000}~psi).
	It is not intended for use with liquids, corrosive gases, or in environments with significant vibration or shock.
	The PTCD maintains its specified orifice diameter across a temperature range of \SI{-60}{\degreeCelsius} to \SI{+80}{\degreeCelsius}, and is calibrated once at room temperature (\SI{25}{\degreeCelsius}).
	It connects to standard gas plumbing with threaded ports and once installed requires no user input or power to operate.
	...
	Standard polytetrafluoroethylene (PTFE) tape is \emph{not} rated for hydrogen service at pressures above approximately \SI{20}{\mega\pascal}. \textbf{Select a sealing method rated for the full operating pressure and verified compatible with your gas.} Consult ASME~B31.3 and your facility's safety officer before making connections.
	\end{warningbox}
	...
	Mount the PTCD on a rigid, level surface. Do not mount on a surface that is independently heated or cooled. Keep at least \SI{15}{\centi\meter} of free air clearance around the actuator on all sides, and make sure no cables, clamps, or insulation contact it. Secure the device firmly enough that it will not move under pipe reaction loads once the system is pressurized.

	\subsection{Ambient Air Conditions}

	The device is designed for quasi-static temperature changes. Avoid directing compressed air, fans, or air-conditioning flow at the actuator, as localized forced convection will cause uneven heating and introduce measurement error. Keep the actuator clear of heat lamps and radiant sources. After placing the device in a new temperature environment, allow at least \textbf{60 minutes} for it to equilibrate before pressurizing or recording data. Fast temperature changes may cause a transient lag between the ambient reading and the actual orifice position.
...
\end{warningbox}
	...
	The PTCD is calibrated at final assembly. \textbf{The PTCD is not designed for disassembly.} Any attempt at disassembly may damage components resulting in unsafe operating conditions. Any modifications are to be done at the user's own risk.
	...
	\begin{tikzpicture}[
		...
		\node[process, below=of install] (equil)
			{(2) Equilibrate temperature\\{\footnotesize(60 min)}};
	...
	\begin{enumerate}[leftmargin=1.8em, itemsep=6pt]
		...
		\item \textbf{Shutdown.} Close the upstream supply valve and open the downstream vent to relieve pressure. Verify depressurization with a gauge. If hydrogen was used, purge the lines before disconnecting any fittings. Repeat as in Figure~\ref{fig:ug_conops}.
...
\label{app:datasheets}
    ...
    316 Stainless Steel\cite{ref_Ledbetter_Stainless}.
		Properties listed are the Coefficient of Thermal Expansion $\alpha$, Thermal Conductivity $k$, Young's Modulus $E$, Poisson's Ratio $\nu$, and Specific Heat Capacity $c_p$.
		Note that the heat capacity of ALLVAR is not certain; this value was provided by ALLVAR Alloys via email as estimated from similar metals.
...
\label{app:PM}
	...
	Development of the \device\ was no small feat. This design, a genuinely difficult engineering challenge that professionals would likely spend a year to developed, was completed in six months.
	The team developed four distinct iterated designs, each time addressing an unavoidable source of error, resolving a manufacturing conflict, or taking advantage of an unforeseen opportunity to make the device simpler and more robust,
	and learning more about engineering design and project management than was thought possible.

	Sandia's design requirements never changed, which prevented unnecessary waste and afforded a streamlined engineering cycle. However, owing to the need for iterative improvements, the project underwent the following major pivots:

	\begin{enumerate}
		\item The initial design architecture, presented in the Preliminary Design Report on February 13, involved a hexagonal orifice composed of six interlocking components, all driven by a single piston (Figure~\ref{fig:hexagon}).
			This mechanism was quickly determined to have too many moving parts, sources of friction, and sources of error to achieve the microscopic diameter and extreme tolerance required.
			An emergency redesign meeting was held on February 23, at which the diamond-overlap orifice was conceived and quickly decided upon as the superior architecture.
			This fundamental shift set the team back several weeks, but it represented the team's realization that this design could not be evaluated on a human scale and that the designs thus far considered
			could not approach the required system output tolerance, no matter how well-designed.
		\item On March 11, nine days before the final design was to be presented in the Preliminary Machine Architecture Report, Sandia management approved disbursement of \$2,000 for the manufacture and testing of a physical prototype.        
			The team quickly drew up a machining and assembly plan to ensure the model would be ready for purchase by the submission deadline.
			They consulted the campus machine shop manager the following day regarding strategies for the manufacture of such small, precise parts. He provided such valuable insight into strategies and off-campus resources for performing the more difficult machining operations
			(i.e. the Electric-Discharge Machining cuts and Electron-Beam-Welded assembly).
			He provided further input on the reliability of the current design. The current model used a double rack-and-pinion (Figure~\ref{fig:gears}) to allow the actuator to sit on its side, saving space along the \device's longest dimension and enabling full satisfaction of Requirement~\specref{R.2}.
			He expressed concerns that the complex mechanical interface would allow no greater precision than the hexagonal orifice, even if manufactured with precision watchmaking components.
			The team judged that a trade-off must be made between Requirements~\specref{R.1} and \specref{R.2}.
			After confirmation of this priority shift with Sandia, the requirement obligation keywords (Section~\ref{sec:intro:req}) were adjusted in compliance with NASA's Systems Engineering Handbook\cite[p.~54]{ref_NASA_specs_guide}, which states that
			``[T]he team must not rely solely on the requirements received to design and build the system. Communication and iteration with the relevant stakeholders are essential to ensure a mutual understanding of each requirement.''
			The geared interface was replaced with a direct connection, supporting the actuator vertically with a rigid mount (Figure~\ref{fig:trap_mount}).
		\item The rigid mount was hastily designed and had to be revised promptly after submission of Preliminary Machine Architecture Report before the \device\ could be manufactured.
			Its initial design, an A-frame structure bolted through flanges on the housing, was unnecessarily complex, difficult to machine, and thermally unstable.
			A simple cylindrical mount (Figure~\ref{fig:app_PTCD}) was devised and its material was replaced with ALLVAR, which had only been recently discovered in the team's research.
		\item As preparations were made to manufacture the prototype, its cost quickly exceeded the provided budget.
			After a series of compromises to reduce the cost, substituting the expensive ALLVAR mount, Invar housing, and bellows with different materials and architectures,        
			Sandia confirmed that the reduced prototype was no longer worth testing.
			The team pivoted to a fully computational model for verification and risk analysis.
			While it may seem this double reversal would have set the project back even further, it motivated necessary changes as the team thought through prototype development from different angles
			and obtained valuable insight from the machine shop manager. 
	\end{enumerate}
	
	Despite its difficulties, development of the \device\ was successful, educational, and invaluable training for every member of the team.
	The project provided its members with their first exposure to real engineering design and equipped them for professional engineering practice better than any individual course could have.
	It will be regarded as the high point in their academic careers.
	...
	\begin{center}
		...
		and to Professor Jonathon Schofield and Teaching Assistant Xiangpu Wang\\
		of UC Davis
