...
\label{chap:design}
	...
	\label{sec:design:just}
		...
		\begin{equation}
			...
			\end{aligned}
		\end{equation}
		\footnotetext{The smaller of the bellows' two necks\cite{ref_miniflex_cataloga}, which is connected to the Valve Orifice.}
...
\begin{enumerate}[label=\roman*.]
  ...
  \item \textbf{Calculations.}
  	...
  	temperature gradient with $\Delta R=D_0(1-a_(alv)*dT_cold/2$)
		$\Delta R = R_0 \left(1 - \left(-\alpha_{\text{alv}}\right)\right) \Delta T_{\text{cold}}$
...
\label{app:sat}
	...
	The tools derived in this chapter provide the means to evaluate this risk. As shown in Section~\ref{app:sat:TF:results}, the \device\ can be expected to meet its required tolerance
	across the full temperature range despite all anticipated sources of error, itemized in Section~\ref{app:sat:error}.
...
\label{app:BOM}
	...
	A quote for the Wire Electrical-Discharge Machining (EDM) operation required to cut each square hole is also included from Milco Wire EDM, Inc; achievable tolerance values given in Section~\ref{app:sat:error} were taken from this quote.
