...
This Design Report \todo{Summary}.
...
\chapter{Del: Final Design Requirements [Copy elements into section \ref{sec:Req}]}
...
\chapter{Del: Detailed Calculations [Copy elements into Appendix \ref{app:calcs}]}
...
\chapter{Del: System Architecture [Copy elements into Section \ref{sec:Arch}]}

\todo{Change the structure to match the template. Keep as much of the below content from Report 2 as needed.}
...
\chapter{Del: Discussion [Copy elements into Section \ref{sec:Risk}}]}
...
expansion and the resisting forces have negligible effect on actuator length.
    ...
    \label{subsec:sys_comp}
        ...
        $\transferFunction$ \quad (eq. \eqref{eq: transfer_function})
        
        which must equal the required relationship

        $\requiredRelationship$ \quad (eq. \eqref{eq: required_relationship})
...
\chapter{Del: Preliminary Bill of Materials [Copy elements into Section \ref{sec:Arch}]}
\label{sec:bom}
