...
\label{chap:design}
	...
	\label{sec:design:just}
		...
		\label{subsec:design:just:P}
			...
			this risk is evaluated in more detail in Section~\ref{sec:DFMEA:P}.
		...
		According to the manufacturer's Design Guide, the burst pressure assumes that the bellows is ``restrained from squirm\footnote{
			Lateral buckling due to high external pressures.
		...
		The valve stem, which passes through the bellows, measures just smaller than the bellows convolution inner diameter (\specBellowsID) to provide support against squirm (i.e. pressure-drive lateral buckling).
...
\section{} % to separate the above subsections from those below
...
\begin{enumerate}[label=\roman*.]
	...
	hand calculations, the Factor of Safety (FOS) can be calculated to determine if the wall is thick enough to withstand the pressure exerted on it.
		...
		\begin{figure}[H]
                \centering
                \includegraphics[width=.55\textwidth]{figures3/Housing Outlet Wall pic.png}
                \caption{Housing Outlet Wall Dimensions.}
                \label{fig:FEAdisp}
            \end{figure}

	\item \textbf{Assumptions, Principles, and Methods.}
	The wall is treated as a circular plate with fixed edges under uniform distributed pressure of 10,000 psi. The maximum stress is calculated using the formula for
	...
	\item \textbf{Results.}
	    \begin{equation}
        \text{FOS} = \frac{S_{y,\,\text{Invar}}}{\sigma_{\max}}
	...
	is not enough to significantly affect the effctive diameter or anything else that would cause the device to fail. 
...
\label{chap:concl}
	...
	\label{sec:conc:fin_anal}
		...
		The Passive Temperature Compensation Device presented in this design report combines proven, robust technology with a novel architecture
		to accomplish thermomechanical flow control with greater precision than has been achieved before.
		By only allowing through the overlap between two diamond holes pressed flush against one another, one mobile and one fixed,
		the \device\ can produce first-order variations in its effective orifice diameter as the mobile square is lowered while only introducing a single moving part.
		These square holes can be manufactured with very sharp corners using wire EDM (Electronic-Discharge Machining).
		An external bimetallic thermal actuator provides compound mechanical displacement to drive these changes.
		Its interface is hermetically sealed with minimal mechanical or fluid resistance using two flexible metal bellows.
		Specialized materials and precise electron-beam welding maximize thermal stability and enable a compact design.
		...
		and customization of the design for particular experiments.

		The design process is incomplete. As intended, only the first iteration was fully developed.
		Computational testing revealed several flaws of the current design but developed helpful tools that will enable more efficient iteration of design changes.
