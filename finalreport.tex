...
\subsection{Introduction to Failure Mode Effect Analysis and Methods}
	...
	inform suggestions for redesign and modification. The results of computational modeling is discussed in \ref{chap:test}	

	\subsection{Weld Failure Analysis}
	...
	\todo{Nick can write about the hand calcs done here for the welds and how if the welds are safe then the "square pressure vessel" that is created.}
	...
	The mount being made of Allvar has an anisotropic quality. It has a positive axial coefficient of thermal expansion that is 2 $\mu$m/m greater than the magnitude of the negative axial expansion 30 $\mu$m/m.
	The largest magnitude of $\Delta R$ occurs over the cold side of the temperature gradient with $\Delta R=(D_0(1-a_alv)*dT_cold/2$

	Is the dR too much
	is welding this part possible

	quick thin wall hand calc
	move on to testing section?
...
\label{chap:concl}
	...
	\label{sec:conc:fin_anal}
		...
		Risk analysis in Chapter~\ref{chap:DFMEA} demonstrated that certain issues will need to be researched and certain revisions made before the \device\ can accomplish its objectives without failure.
...
\label{app:sat}
	...
	The Required Relationship is the core objective of the \device. Concerns are raised in Chapter~\ref{chap:DFMEA} about its ability to meet the subsidiary specifications.
    ...
    \label{subsec:sat:TF:orif}
		...
		\label{sec:sat:TF:results}
			...
			It also serves to validate the results produced by the computational prototype that will be introduced in Chapter~\ref{chap:DFMEA}. \todo{update this conclusion}
