...
\label{chap:Intro}
    ...
    \label{sec:intro:motive}
        ...
        \label{subsec:intro:motive:background}
            ...
            \todo{General motivation: proof of concept to be able to passively accommodate some temperature fluctuations when running these experiments. Why is this needed? Not sure. Get info from Dr. Bon for more details.}
        ...
        \label{subsec:intro:motive:Ops}
            ...
            \todo{Leave the device as a blackbox for now: we don't know how it works yet or even what subsystems it has. But how would a user install and operate a valve that does what we generally described in Objectives?}
...
\label{chap:specs}
    ...
    \label{sec:specs:TF}
        ...
        \begin{multicols}{2}
            ...
            As described in section \ref{chap:arch}, the mount $(M)$ sits atop the housing.
        ...
        \label{subsec:spec:TF:force_balance}
            ...
            \label{subsubsec:spec:TF:force_balance:A}
            ...
            \label{subsubsec:spec:TF:force_balance:S}
            ...
            \label{subsubsec:spec:TF:force_balance:TB}

                $k_T = \specBellowsK $\cite{ref_miniflex_design}

            \subsubsection{Valve Orifice}    
            \label{subsubsec:spec:TF:force_balance:V}
            ...
            \label{subsubsec:spec:TF:force_balance:H}
            ...
            \label{subsubsec:spec:TF:force_balance:W}
        ...
        \label{subsec:spec:TF:pos}
            ...
            \todo{The below is just a draft for now.}

            \begin{align*}
                D_{eff} &= \sqrt{\frac{2}{\pi}} X \\
                &= \sqrt{\frac{2}{\pi}} (y_F - y_R) \\
                &= \sqrt{\frac{2}{\pi}} \left\{\left[L_M - L_A - L_S - \left(\frac{1}{2} - \frac{X_{ref}}{2\ell_{V_0}}\right) L_V\right] - \left[- L_W - \left(\frac{1}{2} + \frac{X_{ref}}{2\ell_{H_0}}\right) L_H\right]\right\}
            \end{align*}
...
\label{chap:sat}
    ...
    \todo{Objectives of this section:
    
        \begin{itemize}
            \item Justify many of our dimensions, quantitatively (e.g. Actuator length, mount wall thickness, orifice height, etc)
            \item Demonstrate system tolerance satisfaction
        \end{itemize}}
...
\label{app:calcs}
    ...
    \label{app:calcs:errorProp}
        ...
        \todo{(Sam) Get rid of the KM stuff and just show the upper and lower bounds, defined as follows:
        
        T>T\_ref:

        Upper bound: positive error on both sides, but A1<0 so A1 -= 5\%, A0 += 5\%
        Lower bound: the opposite. A1 += 5\%, A0 -= 5\%

        T<T\_ref:

        Upper bound: positive error on both sides, but now (T-T\_ref)<0 which negates A1 so both += 5\%
        Lower bound: the opposite, both -= 5\%
        }
