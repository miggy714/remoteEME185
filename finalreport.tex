...
\label{chap:DFMEA}
	...
	\section{Introduction to Design Failure Mode Effect Analysis}
	\label{sec:DFMEA:intro}
		...
		of 100 was chosen to be the threshold for in-depth analysis and potential redesign because, after calculation for each failure mode, it readily separated those that warranted further investigation
		from those that could not be addressed in the time available. Analytical calculations are addressed in the below sections, while
		Chapter~\ref{chap:test} expounds the more involved computational methods used to test certain failure modes.
		...
		For the \device, this includes plastic\footnote{i.e. Irrevocable.} deformation, fracture, rupture, or simply exceedance of the system output tolerance.
		...
		or to inform design modifications. All failure modes identified are listed and identified in Appendix~\ref{app:iter:DFMEA};
		those whose RPN exceeded the threshold value of 100 are listed in Table~\ref{tab:RPN_priority}.
	...
	The welds on the device include the housing welds, actuator, and bellows welds. Note that the housing weld and bellows welds are necessary for sealing and the actuator weld joins the actuator and housing assemblies.
		...
		\label{subsec:DFMEA:weld:main}
			...
			fuse the housing material together without sacrificing yield strength, but this assumption must be further evaluated to ensure the weld can withstand the significant axial internal pressure forces applied.
			...
			\begin{enumerate}[label=\alph*.]
				...
				\item \textbf{Bending stress}: The weld is modeled as a beam fixed at both 
					...
					Additional validation using COMSOL Multiphysics\cite{ref_COMSOL} is ongoing to account for 
...
\subsection{Actuator-Housing Weldment}
    ...
    The weld joints are treated as full penetration fillet welds in this analysis, and the same assumptions made 
  	...
  	temperature gradient using the maximum machined outer radius $R_0 = 9.0215 mm$. It is found that
		...
		\todo{Note from Isaiah, yes it was absolutely wrong, my gosh what the heck was that. result was correct though}
...
\subsection{Orifice Channel Opening}
	...
	See Figure~\ref{fig:FEAdisp2} for tabulated data from the FEA testing that will be used to calculate the FS at the corner of the outlet opening.
	...
	\begin{table}[h]
		...
		\begin{tabular}{lSScc}
		\toprule
		{Analysis Method} & {VM (\si{N/m^2})} & {$S_y$ Invar (\si{N/m^2})} & {Comparison} & {FOS} \\
		\midrule
		SurfMax Outlet Hole     & 2.66E+08 &          & $V_m < S_y$ & 1.04E+00 \\
		SurfAv Outlet Hole      & 7.52E+07 &          & $V_m < S_y$ & 3.67E+00 \\
		SurfMax Outlet Cone     & 3.03E+08 &          & $V_m > S_y$ & 9.12E-01 \\
		SurfAv Orifice Channel  & 1.78E+08 &          & $V_m < S_y$ & 1.55E+00 \\
		SurfMax Orifice Channel & 3.00E+08 &          & $V_m > S_y$ & 9.21E-01 \\
		SurfAv Outlet Cone      & 1.51E+08 &          & $V_m < S_y$ & 1.83E+00 \\
		SurfMax Center Back     & 3.43E+08 & 2.76E+08 & $V_m > S_y$ & 8.06E-01 \\
		SurfAv Center Back      & 1.34E+08 &          & $V_m < S_y$ & 2.05E+00 \\
		SurfMax Center Side     & 2.91E+08 &          & $V_m > S_y$ & 9.49E-01 \\
		SurfAv Center Side      & 5.98E+07 &          & $V_m < S_y$ & 4.62E+00 \\
		SurfMax Center Top      & 1.37E+08 &          & $V_m < S_y$ & 2.01E+00 \\
		SurfAv Center Top       & 7.52E+07 &          & $V_m < S_y$ & 3.67E+00 \\
		SolidMax                & 4.91E+08 &          & $V_m > S_y$ & 5.62E-01 \\
		SolidAv                 & 6.03E+07 &          & $V_m < S_y$ & 4.58E+00 \\
		\bottomrule
		\end{tabular}
	\end{table}
...
\[
    T_{ext}(t) = T_{ref} + \SI{20}{\degreeCelsius}\times \mathrm{tstep}(t),
\]
...
two-hour simulated window (\texttt{range(0, 7200, 60)}). This window was chosen
...
\label{chap:concl}
	...
	Appendix~\ref{app:iter:customize} provides them with guidance on how to use the computational tools presented in Chapters~\ref{chap:DFMEA} and \ref{chap:testing} for the revision before a physical prototype is built,
