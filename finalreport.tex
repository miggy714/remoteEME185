...
\label{chap:Fintro}
	...
	\label{sec:intro:motive}
		...
		\label{subsec:intro:motive:background}
			...
			Therefore, despite this design foundation, a passive, precise valve that continuously manipulates fluid flow without responding to its conditions would prove a novel and highly challenging exercise.
...
\label{chap:design}
	...
	\label{sec:design:overview}
		...
		\label{subsec:design:overview:valve}
			...
			The central moving component, which contains the leading orifice hole, nests between four walls and slides vertically within the assembly.
			This is attached to a valve stem, which extends upwards through the housing to the actuator, which drives its motion.
			...
			\begin{wrapfigure}{l}{2in}
				...
				\includegraphics[width=2in]{orif_assem_right_qual.png}
			...
			As the centerpiece moves up and down, the flexible bellows extend and contract, allowing for flexible, sealed pentration of the housing.
		...
		\label{subsec:design:overview:act}
			...
			\begin{figure}[H]
				...
				\includegraphics[width=.75\textwidth]{Act_cut_qual.png}
			...
			the orifice height will be \SI{10}{\micro\meter} too high at all times, adding a constant \SI{8}{\micro\meter}\footnote{as calculated in Appendix~\ref{app:calcs:orifice_geometry}}
	...
	\label{sec:design:just}
		...
		\label{subsec:design:just:TF}
			...
			we will assume a compressed height of \SI{0.55}{\milli\meter}\footnote{
				...
				Deviations will be accounted for in Chapter~\ref{chap:sat}.}
		...
		Any mechanical penetration of a pressure vessel requires a flexible, sealed interface. Polymer-based gaskets and O-rings are often used, but most such materials permit Hydrogen diffusion and embrittlement in violation of specification \specref{V.3.3}.
		...
		The I718-15-72-430, pictured in Figure~\ref{fig:bellows}, can hold a seal at pressures up to \specBellowsBurst\ with minimal stiffness.
		At the maximum deflection from the reference point (see eq. \eqref{eq:act_def_hot}), the reaction force from the two bellows is only

		\begin{equation}
			F = 2 k \Delta L = 2 \cdot \specBellowsK \specActDefHot = \SI{23}{\newton}
		\end{equation}

		from both bellows using Hooke's Law\cite{ref_Young_Freedman} and the manufacturer's specifications\cite{ref_miniflex_catalog}.
		...
		\begin{equation}
			\begin{aligned}
				F &= P A = (\textit{internal pressure}) \left( \frac{\pi}{4} (\textit{Neck 1 OD\footnote{The bellows' two necks have different diameters; the smaller of those ("Neck 1" in the bellows catalog\cite{ref_miniflex_catalog}) connects to the Valve Orifice}}) \right) \\
				& = (\specPressure) \left( \frac{\pi}{4} (\SI{4.09}{\milli\meter})^2 \right) = \SI{906}{\newton}
			\end{aligned}
		\end{equation}

		The second bellows solves this, adding a maximum of \SI{23}{\newton} from its spring reaction to remove a constant \SI{906}{\newton}.
		...
		The housing inlet allows just enough internal space to fit the bellows, minimizing waste. Regardless, the housing inlet exceeds its specified mechanical envelope to accommodate the two long bellows.
		...
		\label{subsec:design:just:gen_dim}
			...
			\label{subsubsec:design:just:gen_dim:act_diam}
				...
				It should be no surprise that, as calculated in Sections \ref{app:calcs:TF:force_balance:M} and \ref{app:calcs:TF:force_balance:A}, their cross-sectional areas are nearly equal at the optimal distribution.
			...
			\label{subsubsec:design:just:gen_dim:ME}
				...
				As described in detail in Appendix~\ref{app:PM}, Sadia confirmed the precedence of the Required Relationship over the Mechanical Envelope,
		...
		\label{subsec:design:just:LBW}
			...
			\begin{itemize}
				\item Circumferential welds joining the bellows to the housing centers and the valve orifice.
			...
			The geometry of the welds is a square butt joint, which is a widely used in pressure vessel construction, allowing for full penetration of EBW.
			...
			\\~\\
			The viability of the weld joint is evaluated using the ASME B31.3 Process Piping Code 
			...
			\begin{equation}
				S = S_y / 1.5 = \SI{276}{\mega\pascal} / 1.5 = \SI{184}{\mega\pascal}
			\end{equation}
			...
			sufficiently contains the maximum internal pressure with a safety factor of
			...
			($\sigma_\theta = \SI{150.54}{\mega\pascal}$) can be compared directly against the weld joint allowable stress\cite{ref_asme_b313}:
			...
			Electron-beam welding the entire assembly allows for a high-strength pressure-vessel assembly;
		...
		\label{subsec:design:just:mat}
			...
			\label{subsubsec:design:just:mat:Al}
				...
				Aluminum\cite{ref_NASA_specs_Al} stands alone\footnote{
					...
					Its selection process was expidited due to time constraints. Further research may reveal an alloy with a consistent CTE higher than that of aluminum, allowing for a shorter bimetallic actuator.} 
			...
			\label{subsubsec:design:just:mat:Inc}
				...
				INCONEL\textsuperscript{{\textregistered{}}}\cite{ref_SpecialMetals_Inc} Alloy 718, a Nickel superalloy used by Mini-Flex for their high-pressure bellows, outperforms any other metal on several key metrics as a hermitic seal and is ubiquitous in the aerospace industry\cite[p.~1]{ref_SpecialMetals_Inc}.
				...
				\begin{itemize}
					\item Strength at temperature. All metals soften when heated and embrittle when chilled. Inconel, according to its developer, is "suitable for temperatures from [\SI{-250}{\celsius}] up to [\SI{700}{\celsius}]"%
						\footnote{Converted from \textdegree F}\cite[p.~62, 74--76]{ref_Inconel_1965}.
					\item Fatigue resistance. Metals wear under repeated cyclic loads typical of a bellows. Inconel outlasts most metals under such conditions, reported to have "Good tensile, fatigue, creep, and rupture strength" by its manufacturer\cite[p.~1]{ref_SpecialMetals_Inc}.
...
\label{chap:risk}
	...
	reflected in the RPN sheet. To ensure the welds have a sufficient FOS, calculations fed by modeling results were utilized. This process is detailed in Chapter \ref{chap:comp}.
...
\label{chap:concl}
	...
	\label{sec:conc:rec}
		...
		\begin{itemize}
			\item The current design assumes connection to a \SI{19.05}{\milli\meter} pipe, which falls within the range provided in Design Condition~\ref{cond:pipe}, but the design does not readily allow
...
\label{app:calcs}
	...
	\section{Additional Design Justifications}
	\label{app:calcs:just}
