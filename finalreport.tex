...
\label{subsec}
	...
	and are verified or challenged by theoretical modeling in Chapter~\ref{chap:comp}. \todo{fix this ref}
...
\begin{figure}[h]
	...
	reflected in the RPN sheet. To ensure the welds have a sufficient FS, calculations fed by modeling results were utilized. This process is detailed in Chapter \ref{chap:comp}.
...
\begin{enumerate}[label=\roman*.]
  ...
  \item \textbf{Results.}
		...
		$\text{FS} = \frac{S_{y,\text{weld}}}{\sigma_{\text{weld}}}$
...
\begin{enumerate}[label=\roman*.]
	...
	Below is the table of data from the FEA testing that will be used to calculate the FS at the corner of the outlet opening.
...
% ============================================================================
% TESTING CHAPTER  (LaTeX equivalent of Testing.pdf)
% Photos from the PDF are omitted; their intended locations are marked with
% "% [PHOTO]" comments.
% ============================================================================
...
\begin{enumerate}
    ...
    new surfaces which will be referred to as ``symmetry surfaces''. By setting
    ...
    % [PHOTO] Symmetry faces of the half-model PTCD (highlighted symmetry surfaces).
    \item The physics is ``decoupled''. The CFD is run on a rigid negative
    domain of the channel with a turbulent simulation where a ``pressure map''
    ...
    fluid temperature.
    % [PHOTO] Negative (fluid) domain of the channel used for the CFD study.
...
wetted surface of the multi-body simulation.
% [PHOTO] Pressure map / velocity field on the fluid domain (inlet_pressure(5)=10000).
...
by a steady state and transient study.
% [PHOTO] Steady-state surface temperature plot (gas_temp=100, ambient_temp=80).
...
utilized in the DFMEA \todo{reference this chapter for the housing outlet center
wall and the orifice channel opening}.
...
it as a boundary load in the wetted surface \todo{insert reference to figure}.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        the meshes behave. This is shown on the \todo{insert figure reference}
        figure below
    \end{enumerate}
...
DFMEA.
% [PHOTO] MBD result photos (stress on weld joints / failure points).
 
\subsubsection{Limitations}
Talking points:
\begin{itemize}
    \item Friction not modelled
    \item Inconel bellows are modelled with an equivalent material to avoid
    geometric complexity
    \item Allvar is isotropically set to its textbook CTE (Allvar's CTE is
    anisotropic)
    \item The screw and washer are in union instead of being separate parts to
    help with solution convergence
\end{itemize}
 
\subsection{Deformation and Dimensional Tolerance Flow Chart}
% [PHOTO] Volumetric strain and von Mises stress plots
% (gas_temp=45, ambient_temp=25  and  gas_temp=100, ambient_temp=80).
Mapping the transfer function dependencies: CFD $\rightarrow$ MBD $\rightarrow$
Deformation $\leftarrow$ Heat Transfer.
 
\section{Results}
 
\subsection{Design Space Matrix}
\begin{itemize}
    \item % [PLACEHOLDER] design space matrix entry to be added
\end{itemize}
...
% [PHOTO] Normalized orifice diameter across the ambient-temperature design space
% (family of curves for P = 10^2.00, 10^2.67, 10^3.33, 10^4.00 psi).
...
\begin{center}
    \rule{0.35\textwidth}{0.4pt}\quad\textbf{END}\quad\rule{0.35\textwidth}{0.4pt}
\end{center}
...
\label{app:sat}
	...
	\label{app:sat:TF}
		...
		\label{app:sat:TF:G_therm}
			...
			\begin{multicols}{2}
				\begin{align}
					\hat{G}_M &= \ell_{M_0} (1 + \alpha_{\tiny{ALLV}} \left( T_{{eff}_M} - T_{ref} \right) ) \label{eq:G_therm_1}\\
	...
	\label{app:iter:customize}
		...
		\label{app:iter:customize:RR}
			...
			\label{app:iter:customize:RR:slope}
				...
				The slope of system ouput, set to the constant $A_1$ in this design, is defined by the critical actuator dimensions as derived in Section~\ref{subsec:design:just:TF:act}.
