...
\label{sec:Specs}
    ...
    \label{sec:TF}
        ...
        Figure \ref{fig:TF} depicts the full system Transfer Function for the \device. It will take a moment to unpack
        Notice for the general structure: $T_{ambient}$ inputs into a row of sub-functions, representing the thermal expansion of each critical component.
        These sub-transfer functions represent the thermal expansion of each critical component and output the undeformed length $\ell$ of that part at a given temperature.
        Because each bellows applies a spring force against the actuation motion, these values must be evaluated through a force balance via a system of equations, yielding the final length $L$ of each critical component.
        These final values are summed to obtain the position $y$ of each square hole. Taking the difference between these positions, we obtain the height $X$ of the square orifice, which is input to the orifice transfer function $\hat{G}_O$ to obtain $D_{effective}$.
        ...
        Each part is labeled and dimensioned as shown in Figure \ref{fig:positional_block_diagram}:
    ...
    \section{Actuator Subsystem}
        ...
        \begin{figure}[H]
            ...
            \begin{minipage}[t]{0.45\textwidth}
                \todo{Put some of the below text here using minipage}
            \end{minipage}
        \end{figure}
