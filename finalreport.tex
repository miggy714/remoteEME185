...
\label{chap:Fintro}
	...
	\label{sec:intro:req}
		...
		\begin{enumerate}
			\item The device shall passively restrict the provided flow of gas by maintaining the following relationship between its effective orifice diameter\footnote{
				Defined as the diameter of a circle with equivalent area; denoted $D_{eff}$.
			} and the ambient air temperature\footnote{
				The external temperature, not that of the internal gas; denoted $T_{amb}$.
			}:
				...
				\begin{array}{@{} l @{\qquad} l @{}}
					T_{\text{reference}} \text{ (aka } T_{ref}\text{)} = \Tref^{\footnotemark}
						& A_1 = \Aone \\[4pt]
					T_{\text{ambient}}\ \in\ [\Tcold,\Thot]\
						& A_0 = \Azero
				\end{array}
				\]
				\footnotetext{Room temperature}
		...
		The linear nature of the Required Relationship presents particular difficulty. The \device\ must maintain strictly first-order behavior\footnote{
			First-order (i.e. linear) system behavior requires exclusively first-order subsystems, where the output is directly proportional to the output.
			These are exceptionally rare. Mathematician Stanis\l{}aw Ulam famously remarked that the study of nonlinear science is ``like defining the bulk of zoology by calling it the study of `non-elephant animals'.”\cite{ref_nonlinear}.
		} between temperature extremes equivalent to a winter day on Antarctica and a running engine block.
		The following design specifications were carefully selected to support a passive, linear output with micron-scale tolerance.
	...
	\label{sec:intro:specs}
		...
		As depicted in Figure~\ref{fig:NASA_req}, the provided system requirements ``roll down'' into several subsystem-level design specifications that,
		if met, functionally\footnote{
			As defined by NASA's Systems Engineering Handbook, function specifications define the role of each subsystem,
			whereas performance specifications define the precision of that function. As the performance of these subsystems are coupled and cannot be separated,
			performance requirements must be kept at a system level. Thus, if the functional subsystem specifications listed in Section~\ref{sec:intro:specs} are nominally satisfied,
			the functional system requirements will also be satisfied. Performance satisfaction of the Required Relationship will be evaluated in Chapter~\ref{chap:DFMEA}. 
		} guarantee system requirement satisfaction.
		The Required Relationship, Mechanical Envelope, and Design Conditions motivated the following specifications.
		...
		\label{subsec:intro:specs:RR}
			...
			indicating the effective diameter produced by a particular system for any given input temperature. 
			...
			Second, to maintain a first-order control output, each part must exhibit linear behavior. One subsystem cannot correct the nonlinear behavior of another, so any deviation from this rule will contribute to the total system error.
...
\label{chap:design}
	...
	\label{sec:design:just}
		...
		\label{subsec:design:just:P}
			...
			Its yield strength can be readily compared against the Von Mises stress, which measures the distortion energy as a combination of the principle stresses.
		...
		The \device\ requires mechanical penetration of a pressure vessel. Specifications \specref{V.3.2} and \specref{V.3.3} require a hermetic seal\footnote{
			Named after the Greek God Hermes\cite{ref_wilton_hermetic}, a hermetic seal prevents the penetration of any gas above a given leakage rate.
		...
		Hydrogen gas molecules, the smallest in existence, seep through the long polymer chains structures with ease, spurred by an enormous back pressure, and react with the polymer material.
		...
		According to the manufacturer's Design Guide, the burst pressure assumes that the bellows is ``restrained from squirm\footnote{
			Lateral buckling.
		} or sideways movement'' such that ``squirm will not occur if the bellows is guided internally\ldots\ by a rod or stem''~\cite{ref_miniflex_design}.
		...
		\label{subsec:design:just:mat}
			...
			\label{subsubsec:design:just:mat:ALLV}
				...
				rietary, Titanium-based alloy patented by ALLVAR Alloys\cite{ref_AllvarAlloys_gen}.
...
\subsection{Introduction to Failure Mode Effect Analysis and Methods}
	...
	and are verified or challenged by theoretical modeling in Chapter~\ref{chap:comp}.
		...
		\todo{Insert table of prioritized failure modes (RPN>threshold) from drive. Add full sheet to apendix. It is not finalized yet.}
...
\begin{figure}[h]
                \centering
                \includegraphics[width=.75\textwidth]{ Static_FEA_StressMagnitude1.png}
...
textbf{Actuator-Housing Weldament}
\begin{enumerate}[label=\roman*.]
	...
	actuator mount and its' radial weld due to thermal expansion and contraction. This was
	 ...
	 are invar-invar and have relatively low expansion rate constant across all members in
	  the weldment.
 ...
 The mount being made of Allvar has an anisotropic quality. It has a positive axial coefficient
  ...
  expansion 30 $\mu$m/m. The largest magnitude of $\Delta R$ occurs over the cold side of the 
  temperature gradient with $\Delta R=(D_0(1-a_alv)*dT_cold/2$
\Delta R = R_0 \left(1 - \left(-\alpha_{\text{alv}} + 2\times10^{-6}\right)\right) \Delta T_{\text{cold}}
...
\sigma_{\text{weld}} = \frac{E_{\text{alvar}} \cdot \Delta R}{R_0} = \frac{55\times10^{9} \cdot 0.0254 \text{ mm}}{18 \text{ mm}} = 74.86 \text{ MPa}

        \item \textbf{Results.}
The area of the weld and the area of the mount in contact with it are the same, and thus the
 stresses are the same. Given that this stress is much smaller than the yield stress of Alvar 
 and of the weld metal we find a sufficient safety factor:
\text{FOS} = \frac{S_{y,\text{weld}}}{\sigma_{\text{weld}}}
        \item \textbf{Discussion.}
While this FOS is acceptable, this is assuming the weld has the same material properties as
 ...
 detrimental changes in material properties around the weld area which could cause a variety
  of failures. Because Allvar alloy 30 is a experimental alloy with a proprietary composition we 
...
creation of brittle compounds, but this could be highly involved and experimental. A recommended
 path would be to use fasteners with a slight mount redesign that allows for bolting into the
  ...
  /todo{Add in following (purpose, assumpts/principles, results, discussion for all DFMEA topics:
   housing thin wall, orifice, outlet channel)}
...
\addcontentsline{toc}{chapter}{Bibliography}
	...
	\begin{thebibliography}{99}
		...
		\bibitem{ref_nonlinear}	% Non-elephant animals
		D.~K.\ Campbell,
		``Nonlinear science --- from paradigms to practicalities,''
		\textit{Los Alamos Science}, vol.~15, Special Issue: Stanis\l{}aw Ulam
		1909--1984, pp.~218--262, 1987.
...
\label{app:sat}
    ...
    \label{app:calcs:sat:TF:orif}
		...
		\label{app:calcs:sat:TF:results}
			...
			Those that fall outside of tolerance do not exceed their boundary by more than 252\%.
