...
\label{chap:design}
	...
	\label{sec:design:just}
		...
		At the maximum deflection from the reference point (see eq. \eqref{eq:act_def_hot}), a maximum

		\begin{equation}
			F = 2 k \Delta L = 2 \cdot \specBellowsK \specActDefHot = \SI{23}{\newton}
		\end{equation}

		reaction force is exerted from both bellows\cite{ref_Young_Freedman}\cite{ref_miniflex_catalog}.
...
\label{app:sat}
    ...
    \label{subsec:sat:TF:orif}
			...
			\label{subsubsec:sat:TF:orif:G_A}
				...
				as quoted by Milco Wire EDM, Inc (Appendix~\re)
...
\label{app:BOM}
	...
	A quote for the Wire Electric-Discharge Machining (EDM) operation required to cut each square hole is also included from Milco Wire EDM, Inc; achievable tolerance values given in Section~\ref{app:calcs:error} were taken from this quote.
