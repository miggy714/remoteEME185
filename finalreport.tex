...
\label{chap:Fintro}
	...
	\label{sec:intro:specs}
		...
		\label{subsec:intro:specs:RR}
			...
			This process was iterative and depended in part on the architecture selected and described in chapter \ref{chap:design}.
...
\label{chap:design}
	...
	\label{sec:design:overview}
		...
		\begin{figure}[H]
			...
			% \includegraphics[height=\dimexpr\textheight-\pagetotal-3\baselineskip\relax, width=\textwidth, keepaspectratio]{PTCD.png}
		...
		\begin{figure}[H]
			\begin{minipage}{.49\textwidth}
				...
				% \includegraphics[width=.85\textwidth]{exp_qual.png}
		...
		\label{subsec:design:overview:valve}
			...
			\begin{figure}[H]
				...
				% \includegraphics[height=.4\textheight]{Valve_full_qual.png}
			...
			\begin{figure}[H]
				...
				% \includegraphics[width=.8\textwidth]{orif_assem_qual.png}
			...
			\begin{multicols}{2}
				\begin{minipage}{.49\textwidth}
					\begin{figure}[H]
						...
						% \includegraphics[height=4in]{V1_bellows_qual.png}
			...
			\newcolumn
				...
				To hermetically seal these holes, a bellows (5) is welded between the Valve Orifice and each Housing Center (2).
		...
		\label{subsec:design:overview:act}
			...
			\begin{figure}[H]
				\begin{minipage}{.5\textwidth}
					...
					% \includegraphics[height=.4\textheight, width=\textwidth, keepaspectratio]{Act_full_qual.png}
				...
				\begin{minipage}{.5\textwidth}
					...
					% \includegraphics[height=.4\textheight, width=\textwidth, keepaspectratio]{Act_exploded_qual.png}
			...
			The central Aluminum expansion rod (6) nests inside an ALLVAR mount (7), suspended by the screw-washer assembly (8, 9, 10).
...
\end{table}
	...
	\begin{equation}
	FS = \frac{S_y}{\sigma_{vm}} = \frac{276}{190.08} \approx 1.45 > 1
	\label{fs}
	\end{equation}

	verifying that the Housing Inlet and Outlet carry sufficient structural material to hold the maximum specified internal pressure.
	...
	which is sufficiently thick to satisfy its pressure requirements and well within its size constraints.
				...
				The above estimate is concervative and assumes a cylindrical exterior with a wall thickness equal to the thinnest dimension of the actual design.
				Because the valve assembly is rectangular, the additional material provides extra strength.
				...
				\todo{Add comsol results for rectangular center section. The above paragraph can address it.}       
		...
		\label{subsec:design:just:gen_dim}
			...
			The interface between vertical faces of the valve orifice and the housing inlet are subject to frictional forces that could cause wear and galling, which could degrade the system performance over time.
			...
			Compared to uncoated Invar, the TiCN coating provides a 5.6 times increase in hardness and a 63\% reduction in the wear rate under similar conditions\cite{ref_TiCN_Coating}.
			The coating is applied using Physical Vapor Deposition (PVD), a process that deposits a thin film (~ 2--4 \si{\micro\meter}) of material onto the surface of the substrate.
			
			\begin{figure}[H]
				\centering
				\framebox[0.85\textwidth]{\vbox{\vspace{1.5cm}
					\hbox{\centering \textbf{[ PLACEHOLDER: COATED VALVE SURFACES ]}}
					\vspace{0.3cm}
					\hbox{\centering \small This figure will show the specific internal mechanical boundaries}
					\hbox{\centering \small and sealing interfaces where the TiCN coating is applied to the Invar substrate.}
				\vspace{1.5cm}}}
		...
		\label{subsec:design:just:LBW}
			...
			The geomtry of the welds is a square butt joint, which is a widely used in pressure vessel construction, allowing for full penetration of EBW.
			...
			\cite[Table 302.3.4]{ref_asme_b313}.
			...
			(Equation \eqref{tmin_weld}, below)\cite[eq.~3a]{ref_asme_b313}, which governs the required minimum wall thickness as a function 
			of the weld joint quality factor $E$. Note that the calculations below use a cylindrical pipe geometry as a conservative approximation of the rectangular housing geometry, as the welds are located at the inner wall of the housing where the stress is highest and the wall thickness is lowest.
			...
			Per ASME B31.3 \cite[Table 302.3.4]{ref_asme_b313}, the weld joint quality factor $E$ depends entirely 
			...
			The design wall thickness, $t_{actual} = \SI{7.806}{\milli\meter} > t_{min} = \SI{6.52}{\milli\meter}$,
			is sufficiently contains the maximum internal pressure with a safety factor of
			...
			\begin{equation}
			SF_{stress} = \frac{\sigma_{allow}}{\sigma_\theta} = \frac{\SI{184}{\mega\pascal}}{\SI{150.54}{\mega\pascal}} \approx 1.22 > 1
			\label{sf_stress}
			\end{equation}

			These results confirm that, if all welds pass 100\% X-ray radiographic inspection ($E = 1.00$)\cite{ref_asme_b313}, 
			...
			Electron-beam welding the entire assembly allows for high-strength pressure-vessel assembly;
			it also enables an assembly with minimal degrees of freedom and a direct interface between the two subsystems,
			improving the system-level precision by preventing backlash allowed by alternative assembly methods.
...
\label{chap:risk}
	...
	The high performance requirements and extreme operating conditions of this device require in-depth risk assessment via Failure Mode Effect Analysis (FMEA). Possible failure modes are identified
	...
	by taking the product of the qualitative ranking of individual factors of failure severity, probability of occurence, and ease of detection. A critical RPN
	of \todo{critical RPN} was chosen to be the threshold for in-depth analysis and potential redesign. Note that these failure modes were identified and ranked through preliminary calculations
	and are verified or challenged by theoretical modeling in chapter \ref{chap:comp}.

		\todo{Insert table of prioritized failure modes (RPN>threshold) from drive. It is not finalized yet.}

	The threshold RPN was chosen to be \todo{critical RPN}. Since human operators do not come into direct contact or operate the PTCD directly, the main failure risks 
	do not include human injury, but rather device inaccuracy, loss of function, and damage to the PTCD and nearby equipment at the worst case.
	This RPN value was deemed the critical point at which inaccuracies, loss of function, or damage would prove the device incapable of performing within an acceptable
	tolerance range \todo{if we have time/if possible, see what worst case deformation in critical components can be via transfer function}
	


	Of the critical failure modes \todo{number}? of them are on welds. \todo{finish weld calculations, insert calcs in chapter \ref{chap:comp}}
	The electron beam welding serves to seal the bellows to the housing components, join the housing assembly, and fixture the actuator assembly to the housing.
	...
	reflected in the RPN sheet. To ensure the welds have a sufficient FOS, calculations fed by modeling results were utilized. This process is detailed in capter \ref{chap:comp}.
...
\label{chap:comp}
	...
	\todo{Computational Model: all of our numerical tests and their results go here. Must include details about how we went about setting up the virtual prototype. This will likely be spliced with chapter \ref{chap:risk}. Follow steps 1-6 above \textit{verbatim}.}
...
\label{chap:concl}
	...
	\begin{wrapfigure}{l}{1.5748in}
		\centering
		\includegraphics[width=1.5748in]{act_size.png} % IMPORTANT: Do not change the size of this image. It is printed to-scale. Delete if necessary.
	...
	\begin{figure}[H]
		\centering
		\includegraphics[height=3in]{reveal.png}
		% \includegraphics[height=3in]{reveal_qual.png}
	...
	\label{sec:conc:fin_anal}
		...
		Modern defense technology research requires ever-increasing precision to maintain a competetive edge in the international theater.
		Gas flow experiments conducted by Sandia National Labs 
...
\label{app:BOM}
	...
	\todo{EDM tols not included in quote yet; waiting for Xometry to respond}
...
\chapter{Manufacturing and Operation}
	...
	Invar hardens as it is machined. Cut with slow, shall passes and ample cutting fluid.
	...
	\begin{enumerate}[itemsep=4pt]
		...
		\item \textbf{Tap large hole} with a \nicefrac{3}{4}-14\,NPT pipe thread tap. Use cutting fluid.
	\end{enumerate}
	...
	\begin{enumerate}[itemsep=4pt]
		...
		\item \textbf{Mill valve stem}. From several direction perpendicular to the vertical axis,
					...
					Leave a few mm clearance above and below the rectantular section.
...
Mount the PTCD on a rigid, level surface. The actuator must be vertical and tilting the device introduces an unintended gravitational load on the valve stem. Do not mount on a surface that is independently heated or cooled. Keep at least \SI{15}{\centi\meter} of free air clearance around the actuator on all sides, and make sure no cables, clamps, or insulation contact it. Secure the device firmly enough that it will not move under pipe reaction loads once the system is pressurised.
