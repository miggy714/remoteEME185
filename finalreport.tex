...
\newcommand{\specInvarCTE}{\SI{1.3e-6}{\per \celsius}}
...
\newcommand{\GA}{\SI{1.91}{\frac{\micro \meter}{\celsius}}}
\newcommand{\GV}{\num{-0.798}}
	...
	\label{sec:design:just}
		...
		\label{subsec:design:just:TF}
			...
			\newcolumn
				...
				See Figure \ref{fig:L_X}. Thus, if the mount shrinks, and the expansion rod grows, by 1 \textmu m each, the length $L$ will increase by 2 \textmu m.
				This drives the valve stem an equal amount, lowering the valve stem and shrinking the orifice height $X$ by 2 \textmu m.
			...
			\label{subsubsec:design:just:TF:valve}
				...
				\begin{equation*}
					\SI{40.201}{mm} - \SI{5}{mm} = \SI{35.201}{mm}
				\end{equation*}
				...
				The bottom corner of the trailing orifice lies $\SI{35.519}{mm}$ below the same surface, resulting in an effective diameter of
				\todo{Verify the two dimensions above once drawings are finished}

				\begin{equation}
					D_{ref} = \sqrt{\frac{2}{\pi}} X_{ref} = \sqrt{\frac{2}{\pi}} (\SI{35.519}{mm} - \SI{35.201}{mm}) = \AzeroExact
				\end{equation}
			...
			\label{subsubsec:design:just:TF:act}
				...
				A belleville washer is compressed between the upper interior of the mount and the top surface of the actuator, held in place by the screw. Uncompressed, it measures 0.6 mm tall; we will assume a compressed height of 0.55 mm\footnote{The exact compression height of the belleville washer is, by nature an unknown and varies between each device. If the washer needs to be compressed more or less than assumed, the deviation from the expected expansion rate will be no greater than that already introduced by the machining error.}
				A 5mm gap, spanned by the valve stem, will separate the free end of the actuator from the housing, thermally insulating the actuator from direct heat transfer with the hot internal gas.
				...
				\begin{multicols}{2}
					...
					the expansion rod (aka the "actuator") will expand townwards as it is lifted upwards when heated. This action yields the actuation-temperature relationship in equation \eqref{eq:actuation}.
				...
				\begin{align}
					...
					&= \frac{\GA + (5.55\ mm) \cdot (\specMountCTE)}{\specActuatorCTE - \specMountCTE} = \specOrigActuatorLength \\[12pt]
					L_{mount} &= L_{act} + t_{wash} + L_{gap} = 38.541\ mm
				\end{align}

				These critical dimension ensure that the actuator subsystem maintains the specified output
		...
		\label{subsec:design:just:geo_sel}
			...
			\todo{Justification for the valve architecture: linear, minimal dof, simple. Briefly mention alts. Justify direct contact.}
			...
			$\hat{G}_{RR}$'s tolerable error is limited to 5\% of $A_1$, or $\SI{76.2}{\frac{nm}{\degC}}$, a razor-thin margin.
		...
		\label{subsec:design:just:bellows}
			...
			\todo{All bellows information. Satisfies H-comp.}

			\begin{multicols}{2}
				\begin{minipage}{.49\textwidth}
					\begin{figure}[H]
						\centering
						\includegraphics[width=\textwidth]{bellows.png}
			...
			\newcolumn
				...
				The I718-15-72-430, pictured in Figure \ref{fig:bellows}, can hold a seal at pressures up to 

				The bellows drove many of the surrounding dimensions and design features.
				According to the manufacturer's Design Guide\ref{ref_miniflex_design}, the burst pressure assumes that the bellows is "restrained from squirm or sideways movement" such that "squirm will not occur if the bellows is guided internally... by a rod stem".
		...
		\label{subsec:design:just:gen_dim}
			...
			\label{subsubsec:design:just:gen_dim:ME}
				...
				As stated in various sections above, each subsystem met its mechanical envelope specifications, with one exception: the valve subsystem violates its vertical mechanical envelope, pushing the total system height to $(45.55 + 70.72) = \SI{116.27}{mm}$, a 16\% violation of the required mechanical envelope.
		...
		\label{subsec:design:just:cal}
			...
			The manufacturer will need to calibrate each device after assembly to correct for any constant-offset error introduced by machining and assembly errors.
			This requires only a low-pressure operation test of room-temperature air.
			The user can pump room-temperature air at a couple hundred kPa (2-3 atmospheres) while measuring the upstream pressure, downstream pressure, and ambient air temperature precisely.
			The required orifice diameter will correspond to a given pressure drop.
			While reading live measurements, he can adjust the screw as necessary.
			The belleville washer will maintain pressure between the two actuator cylinders, allowing the user to precisely set the orifice at this temperature and maintaining that setpoint.

			The torque required to tighten the screw is calculated in Appendix \ref{app:calcs:belle}.
		...
		\label{subsec:design:just:mat}
			...
			\label{subsubsec:design:just:mat:Al}
				...
				Given the demanding constraints for the actuator subsystem proper choice of materials is crucial. 
				Device passivity requires precision actuation without correction or sensing (mention the +/-15um again?). 
				The actuator must be sensitive to changes in temperature with minimal difference in function across 
				the operating temperature range. These conditions paired with force and deflection calculations showed that 
				hydraulics and pneumatics could not be considered due to insufficient CTE and unwanted compression. 
				Polymers show promising high CTE’s and some with sufficient stiffness, but have properties that vary greatly 
				across the operating temperature range that could surface in error or failure. Ceramics do not display as 
				high if CTEs though their stiffness and property stability are good. Their low machinability and brittleness 
				narrows our choice down to metals.  Aluminum was chosen as it has relatively high and constant CTE
				and sufficient stiffness. The choice of Allvar for the second material in the bimetallic component 
				compliments aluminum’s positive CTE with a greater negative CTE. Allvar like aluminum showcases CTE 
				stability and excellent stiffness. These two metals paired offer a solution that can deliver long 
				and accurate stroke lengths across our temperature range with negligible deflection under stress.

			\subsubsection{Allvar mount}
			\label{subsubsec:design:just:mat:ALLV}

				\todo{ALLVAR justification}

			\subsubsection{Invar housing}
			\label{subsubsec:design:just:mat:Inv}

			Invar 36$^{\textregistered{}}$, used for all machined parts in the valve subsystem, exhibits an exceptionally low coefficient of thermal expansion.
			Swiss physicist and metrologist Charles \'{E}douard Guillaume developed the exotic metal Invar, short for "\textit{invariable}", in 1897\cite{ref_Invar_1897}.
			...
			Carpenter Technology\cite{ref_Carpenter_gen}, a manufacturer of the alloy, now holds the registered trademark to the name Invar 36 and confirms that its CTE reaches no more than $\SI{1.3}{\frac{\mu m}{\degC}}$ within our temperature range\cite{ref_Carpenter_Inv},
			...
			INCONEL$^{\textregistered{}}$ Alloy 718, the material used by Mini-Flex for their high-pressure bellows, is an exceptionally strong metal alloy.
			Special Metals Corporation\cite{ref_SpecialMetals_gen} holds the registered trademark.
			\todo{material props}\cite{ref_SpecialMetals_Inc}
...
\begin{thebibliography}{99}
	...
	\bibitem{ref_SpecialMetals_Inc} % Inconel
	...
	\bibitem{ref_McMaster_belle} % Standard washer part specificaitons
	...
	Carpenter Technology Corporation, \textit{Carpenter Technology: Global Leader in Specialty Alloys}, Carpenter Technology Corporation, 2026. [Online]. Available: \url{https://www.carpentertechnology.com/}
	[Accessed: May 8, 2026].

	\bibitem{ref_SpecialMetals_gen}
	Special Metals Corporation, \textit{Special Metals: High-Performance Alloy Products}, Special Metals Corporation, 2026. [Online]. Available: \url{https://www.specialmetals.com/}
	[Accessed: May 8, 2026].
...
\label{app:calcs}
	...
	\label{app:calcs:belle}
		...
		This paper assumes a deflection of 50 \textmu m and, if all parts are machined and assembled as specified within tolerance, the applied deflection will be near this value.
		However, the washer can deflect up to 200 \textmu m if necessary.
		...
		\begin{itemize}
			\item Outer Diameter $OD = \SI{5}{mm}$
			\item Inner Diameter $ID = \SI{2.2}{mm}$
			\item Young's Modulus $E = \SI{207.8}{GPa}$
			\item Poisson's Ratio $\nu = 0.282$
			\item Free height $H = \SI{0.6}{mm}$
			\item Thickness $t = \SI{0.4}{mm}$
			\item Cone height $h = H - t = \SI{0.2}{mm}$
			\item Deflection $0 \leq \delta \leq \SI{0.2}{mm}$
		\end{itemize}
		...
		yielding an expected torque of $\SI{1.7e-3}{N-m}$ at the 50 \textmu m compression, a very reasonable value for an M2 screw.
...
\label{app:datasheets}
	...
	The following information is provided on the following pages:
	...
	\section{Bellows Specifications}
		...
		Note that although the squirm pressure rating, \specBellowsSquirm, is less than the pressure capacity stipulated by Specification \specref{V.3.2} (\specPressure), the Design Guide states that this value assumes no 
...
\todo{Put material properties with references here as a catch-all.}
