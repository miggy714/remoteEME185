...
\label{sec:Req}
    ...
    \todo{Ask Dr. Schofield about only using 2 reqs, not 3, since we only have that many}
    ...
    \todo{Update the below requirements}

    Below are the subsystem specifications needed to satisfy the Required Relationship:

        $$D_{\text{eff}} = A_1(T - T_{\text{ref}}) + A_0$$

    which corresponds to the system transfer function in equation \eqref{eq: transfer_function}:

        $$D_{\text{eff}}=\frac{dD_{\text{eff}}}{dX} \left[\frac{dL}{dT}\frac{dX}{dL}(T-T_{\text{ref}})+X\left(T_{\text{ref}} \right) \right]$$

    \todo{TF above is out of date}
