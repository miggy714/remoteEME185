		...
		\label{subsec:intro:specs:RR}
			...
			\begin{multicols}{2}
				...
				\begin{figure}[H]
					...
					\caption{Subsystem components of $\hat{G}_{RR}$}
					\label{fig:TF_sub}
				\end{figure}
				\newcolumn
	...
	\label{sec:design:just}
		...
		\label{subsec:design:just:TF}
			...
			we will assume a compressed height of \SI{0.55}{\milli\meter}\footnote{
				...
				Deviations will be accounted for in Chapter~\ref{chap:risk}.}
		...
		\label{subsec:design:just:geo_sel}
			...
			Although it was an entirely novel approach to this problem, the design fit all three selection criteria and, as demonstrated in Chapter~\ref{chap:risk}, ultimately satisfies the Required Relationship.
		...
		\label{subsec:design:just:P}
			...
			Principal Stresses\cite{ref_shigley}:
			\begin{equation}
			...
			\todo{Add comsol results for rectangular section in above paragraph}       
		...
		\label{subsec:design:just:gen_dim}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=.75\textwidth]{coated_surfaces.png}
		...
		\label{subsec:design:just:LBW}
			...
			EBW's high power density enables complete fusion across the interface.
			Weld penetration quality is verified using 100\% X-ray radiographic inspection per ASME B31.3 
			\cite[Table 302.3.4]{ref_asme_b313}. If all welds pass inspection ($E = 1.00$)\cite{ref_asme_b313}, 
			...
			Electron-beam welding the entire assembly allows for an assembly with minimal degrees of freedom and a direct interface between the two subsystems,
			improving the system-level precision by preventing backlash allowed by alternative assembly methods.
			\\~\\
			The viability of the weld joint is evaluated for both axial and bending stresses due to the internal pressure.
			...
			\begin{table}[H]
				...
				\begin{tabularx}{\textwidth}{l X l}
					...
					\\\addlinespace
					$A_{wetted}$ &
					Wetted area (From gas flow)&
					\SI{3.5504e-4}{\square\meter}
					\\\addlinespace
					$A_{weld}$ &
					Weld area &
					\SI{1.3508e-3}{\square\meter}
					\\\addlinespace
					$S_y$ &
					 Yield strength &
					\SI{276}{\mega\pascal}
			...
			The axial stress on the weld is calculated as the force from the internal pressure distributed across the weld area \cite{ref_shigley}.

			\begin{equation}
			\sigma_{axial} = \frac{F}{A} = \frac{P A_{wetted}}{A_{weld}} 
			...
			\begin{equation}
			FS_{axial} = \frac{S_y}{\sigma_{axial}} = \frac{276}{18.08} \approx 15.27 > 1
			\label{sf_axial}
			\end{equation}

			Since the axial stress is well below the yield strength, the weld joint can withstand the internal pressure without failure in tension.
			
			The bending stress is modeled  using the formula for bending stress in a beam fixed at both ends where $M_{max}$ is the maximum bending moment (Equation \ref{moment_bending}, at $L/2$), $W$ is the distributed load from the internal pressure, $L$ is the length of the weld, $b$ is the width of the weld, $t$ is the thickness of the weld, $c$ is the distance from the neutral axis to the outer fiber, and $I$ is the moment of inertia of the weld cross-section.
			The beam approach is a simplification that assumes the weld behaves like a beam under bending loads \cite{ref_beam}. In this case, the valve geometries are too complex to model the bending stress accurately, yet Equation \ref{stress_bending} shows that the bending stress can be minimized by decreasing $L$ and/or increasing $t$.
			\begin{equation}
			M_{max} = \frac{W L^2}{24} = \frac{(P b) L^2}{24}
			 \label{moment_bending}
			\end{equation}

			\begin{equation}
			\sigma_{bending} = \frac{M_{max} c}{I} = \frac{M_{max} (t/2)}{(1/12) b t^3} = \frac{6 M_{max}}{b t^2} = \frac{6 (P b) L^2}{24 b t^2} = \frac{P L^2}{4 t^2}
			\label{stress_bending}
			\end{equation}

			Further modeling of this joint was performed using COMSOL Multiphysics to verify that the bending stress does not exceed the yield strength of the material, and to account for the complex geometry of the weld joint.

			\todo{ADD COMSOL MODELING  and/or Replace bending stress with Roark formulas}
...
\label{chap:risk}
	...
	\end{figure}
		...
		\subsection{Failure Mode and Effect Analysis Process}
	...
	tolerance range \todo{if we have time/if possible, see what worst case deformation in critical components can be via transfer function}
		...
		\subsection{Weld Analysis}

	Of the critical failure mod
	es \todo{number}? of them are on welds. \todo{finish weld calculations, insert calcs in Chapter~\ref{chap:comp}}
	...
	In addition to stresses applied by the internal pressure, there is also stress on the actuator mount and its' radial weld due to thermal expansion and contraction.
	The mount being made of Allvar has an anisotropic quality. It has a positive axial coefficient of thermal expansion that is 2 $\nu$m/m greater than the magnitude of the negative axial expansion 30 $\nu$m/m
...
\label{chap:concl}
	...
	\label{sec:conc:fin_anal}
		...
		National Security demands ceaseless advances in high-performance technology to maintain a competitive edge in the international theater.
		The Passive Temperature Compensation Device fills the niche of its active-control counterparts with precision and reliability,
		providing Sandia National Laboratories with a consistent, independent means of experimental conditioning
		and eliminating that parametric uncertainty from their defense technology development process.
		Without the PTCD, Sandia must either depend on active-control mechanisms, introducing an unnecessary variable in their regulation of experimental parameters,
		or perform extensive post-processing when strict power and computation constraints prohibit such intervention.

		Several key design innovations enabled the \device\ to accomplish a previously impossible mission: precise, continuous, high-pressure, thermomechanical flow control.
		Proven, aerospace-grade technology combined with a novel diamond-orifice valve constitute a simple, linear architecture capable of maintaining a pin-size orifice with medical precision. 
		Precise electron-beam welding and two flexible metal bellows maintain a hermetic seal under extreme pressures.
		Specialized materials allow for a compact, thermally stable housing and consistent, thermomechanical actuation.
		Minimizing the system's degrees of freedom to a single moving part eliminates critical error;
		belleville calibration corrects much of what remains.
		Finally, electrical-discharge machining enables the manufacture of the variable-diamond orifice at the heart of the \device,
		producing a sharp, linear temperature-diameter output.
		This proof-of-concept model provides Sandia with the necessary foundation for development of a working prototype.
		...
		However, these adjustments are minor and will likely not require fundamental revision of the design architecture.
		While this report does not provide a finished product, it develops, demonstrates its viability as a functional device, and provides convenient tools for iterative improvement and verification.
	...
	\label{sec:conc:rec}
		...
		\begin{itemize}
			...
			\item As mentioned in Section~\ref{subsubsec:design:just:gen_dim:ME}, certain dimensions can be optimized to decrease the Mechanical Envelope if necessary.
				The verification tools developed in Chapter~\ref{chap:comp} provide a convenient method to quantify the impact of these changes on system output precision.
				\begin{itemize}
...
\addcontentsline{toc}{chapter}{Bibliography}
	...
	\begin{thebibliography}{99}
		...
		\bibitem{ref_beam}
		A. Grishin, ``Thin-Wall Rectangular Pressure Vessels Simulation,'' Phoenix Analysis \& Design Technologies (PADT), Inc., Tempe, AZ, USA, White Paper, Jun. 30, 2024. Available: \url{www.padtinc.com}. [Accessed: May 29, 2026].
...
\label{app:sat}
	...
	The Required Relationship is the core objective of the \device. Concerns are raised in Chapter~\ref{chap:risk} about its ability to meet the subsidiary specifications.
	The tools derived in this chapter provide the means to evaluate this risk. As shown in Section~\ref{app:calcs:TF:results}, the \device\ can be expected to meet its required tolerance
	across the full temperature range despite all anticipated sources of error, itemized in Section~\ref{app:calcs:error}.

	\section{Error Analysis}
	\label{app:calcs:error}

		\subsection{Variations in Aluminum's Coefficient of Thermal Expansion}
		\label{app:calcs:error:CTE}
	...
	\label{app:calcs:TF}
		...
		\subsection{Heat Transfer}
		\label{app:calcs:TF:dT}

			\todo{Heat transfer analysis from COMSOL testing}
		...
		\label{app:calcs:TF:G_therm}
			...
			The CTE of aluminum is computed as a function of temperature as defined in Section~\ref{app:calcs:error:CTE} to find the impact of this error on the final output.
    ...
    \label{subsec:sat:TF:orif}
		...
		\label{sec:sat:TF:results}
			...
			These results must be taken with skepticism.
			...
			It also serves to validate the results produced by the computational prototype that will be introduced in Chapter~\ref{chap:comp}. \todo{update this conclusion}
...
\label{app:BOM}
	...
	A quote for the Sinder EDM operation required to cut each square hole is also included; achievable tolerance values given in Section~\ref{app:calcs:error} were taken from this quote.
