...
\label{chap:Fintro}
	...
	\label{sec:intro:specs}
		...
		\label{subsec:intro:specs:RR}
			...
			into its specified subcomponents allows for an unconstrained enumeration of values so long as each architecture supports a linear, precise output.
...
\label{chap:design}
	...
	\label{sec:design:just}
		...
		\label{subsec:design:just:geo_sel}
			...
			in Section~\ref{subsec:intro:specs:RR}. $A_1$ is expressly first-order. Any nonlinear behavior in one subsystem must be either reparameterized in the other subsystem, an imprecise process involving a complex mechanical interface, or tolerated as error.
		...
		The I718-159-72-430, pictured in Figure~\ref{fig:bellows}, can hold a seal at pressures up to \specBellowsBurst\ with minimal stiffness.
		At the maximum deflection from the reference point (see eq. \eqref{eq:act_def_hot}), a maximum

		\begin{equation}
			F = 2 k \Delta L = 2 \cdot \specBellowsK \specActDefHot = \SI{24}{\newton}
		\end{equation}

		reaction force is exerted from both bellows\cite{ref_Young_Freedman, ref_miniflex_catalog}.
		...
		\label{subsec:design:just:gen_dim}
			...
			\label{subsubsec:design:just:gen_dim:act_diam}
				...
				Though imperceptible, the actuator deflection can be easily minimized and optimized by increasing the cross-sectional area of each component properly.
			...
			\label{subsubsec:design:just:gen_dim:ME}
				...
				but the tools since developed and presented in Section~\ref{app:sat:TF} readily allow for a more informed trade-off analysis.
...
\subsection{Introduction to Failure Mode Effect Analysis and Methods}
	...
	inform suggestions for redesign and modification. The results of computational modeling are discussed in \ref{chap:test}	
...
\label{chap:concl}
	...
	\label{sec:conc:fin_anal}
		...
		While this report does not provide a finished product, it develops the design, demonstrates its viability as a functional device, and provides convenient tools for iterative improvement and verification.
		...
		to accommodate individual experiments. Fine adjustment of the actuator, mount, and valve stem lengths allows for a fully customizable Required Relationship for further iterations. 
...
\label{app:calcs}
	...
	\label{app:calcs:cal}
		...
		\label{app:calcs:cal:sat}
			...
			As discussed in Section~\ref{app:calcs:sat:TF:results}, this calibration stack will occasionally be necessary to correct static machine dimension errors,
...
\label{app:sat}
	...
	\label{app:calcs:TF}
		...
		\label{app:calcs:TF:G_therm}
			...
			Dimensional errors are stochastically added or subtracted to each machined part length $\ell_0$. This process is discussed in more detail in Section~\ref{app:calcs:sat:TF:results}. 
    ...
    \label{app:calcs:sat:TF:orif}
			...
			\label{app:calcs:sat:TF:orif:G_A}
			...
			\label{app:calcs:sat:TF:orif:G_D}
		...
		\label{app:calcs:sat:TF:results}
			...
			are stochastic. Producing a normalized distribution expectation plot via Monte Carlo analysis\cite{ref_figliola},
			...
			This System Transfer Function does not purport to reliably simulate the full performance of the \device.
			...
			The function does, however, provide an inexpensive tool to holistically analyze the expected behavior of various parameter adjustments, such as changes in material properties, machined dimensions, expected error values, and calibration setpoints.
...
\label{app:BOM}
	...
	A quote for the Wire Electrical-Discharge Machining (EDM) operation required to cut each square hole is also included from Milco Wire EDM, Inc; achievable tolerance values given in Section~\ref{app:calcs:error} were taken from this quote.
...
\chapter{Manufacturing and Operation}
	...
	\begin{notebox}
		...
		\begin{itemize}[itemsep=2pt]
			\item Computer-Numeric Control (CNC) mill and lathe that can meet or exceed the dimensional
						tolerances in Appendix~\ref{app:drawings}
			\item Band saw
	...
	Before beginning, familiarize yourself with every part using the figures below
	...
	This guide is written for test engineers and laboratory technicians who operate high-pressure gas experiments. Users are assumed to have familiarity with pressurized gas systems and applicable codes. 
...
\end{warningbox}
	...
	Depressurize the system before calibrating. Measure the orifice diameter at $T_\mathrm{ref} = \SI{25}{\degreeCelsius}$ using the sponsor-provided optical procedure and calculate the required height correction. Adjust the T6 adjustment screw in minute increments to achieve the target effective diameter. 
	\begin{warningbox}
