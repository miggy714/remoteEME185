% Gentlemen, congratulations on finishing this report!
...
% P.S. Focus especially on making the architecture description clear in section 2
% and verifying our calculations in Section 3.
...
\label{sec:system_architecture}
    ...
    \label{sec:initial_system_requirements}
        ...
        \begin{equation}
            D_{\text{effective}}=\sqrt{\frac{4A}{\pi}}
            \label{eq:d_eff}
        \end{equation} 
    ...
    \label{sec:sys_arch_overview}
        ...
        \end{equation}
         ...
         as calculated in Appendix \ref{app:orifice_geometry}. The control derivative $\frac{dD_{eff}}{dX}$ is governed by the valve subassembly.
    ...
    \label{sec:act_sub_description}
        ...
        \end{figure}
            ...
            % Paragraph 2
            The actuator block is mounted between the top inside face of the mount and valve stem. As the ambient temperature rises, the actuator expands downwards pushing the valve stem and translating the plate downward to close the orifice. Similarly, as ambient temperature decreases, the actuator contracts causing the plate to be pulled up, therefore reopening the orifice. The actuator is directly coupled onto the valve stem.
            Furthermore, the full thermal expansion of the block across the 140 $\degC$ operating range produces 267 $\mu$m of stroke. This motion is done purely by the actuator interfaced with the valve stem. There is no other linkage, gear train, or motion amplification that occurs.
            The block interfacing with the valve stem directly eliminates possible hysteresis and eases the manufacturing process. 
...
\label{sec:discussion}
    ...
    \section{Valve Subsystem}
        ...
        \label{subsec:valve_spec_2}
                ...
                Note that within the valve stem the total deflection will be that of thermal expansion minus that of compression: $5.0975-2.45 = -2.6475 \text{ microns}$. This is just lower than the specificaton of 2.75 $\mu$m.
    ...
    \subsubsection{Specification 1: Thermal Expansion Tolerance}
        \label{subsec:act_spec_1}
            ...
            $\delta L {(T_{ref} \to T_{max})} = L_0 \alpha \Delta T = (84.335 \text{ mm})(22.7 \times 10^{-6})(70) = 0.1339 \text{ mm}$  
...
expansion and the resisting forces have negligible effect on actuator length.
    ...
    \section{Mount Subsystem}
     ...
     \subsection{Overview}
            ...
            % Paragraph 2
            The crux of the matter is that the actuator block is anchored at the top interior face of the mount, and the free end extends downward to contact the valve stem. As the ambient temperature changes, the aluminum actuator
        ...
        \label{subsec:mount_spec_2}
            ...
            After calculation of the expected post-assembly value of $X(T_{ref})$, we can increase the height of the mount by one of three values:
        ...
        \todo{Fill in something small here.}
        ...
        \subsubsection{Trapezoidal Mount Plate Requirement Updates}
            The system-level mechanical envelope requirement was ($\leq$ 10 $cm^3$) and the stiffness requirement both roll down directly to the mount plates. The plates must be tall enough to house the actuator block in the cavity between them, wide enough such that base can cradle the valve house housing,
        ...
        In addition to thermal expansion, the mount plate thickness is an important consideration. The plates must be thick enough such that the actuator's reaction force or response to pressure does not cause any mechanical deflection. The actuator exerts a maximum force
         ...
         Given that Invar 36 has a modulus of 140 GPa and the mount plates have substantial cross-sectional area at their base, the deflection under such a modest load is expected to be negligible relative to the 162 µm stroke from $T_{ref}$, but this will be quantitatively confirmed before fabrication.
        ...
        \[ F_{bolt} = 14.2 mm^2 \times 9.7 \times 10^8 {N/mm^2} = 13.774 \ \text{Giganewtons} \]

        For a total of eight screws:
         \[ F_{total} = 8 \times 14.2 mm^2 \times 9.7 \times 10^8 {N/mm^2} = 110.192 \ \text{Giganewtons} \]
...
is can be related to the effective diameter as follows:

    $$ A = \frac{\pi}{4}D_{eff}^2 = s^2 $$
    $$ s = \sqrt{\frac{\pi}{4}}D_{eff} $$
    $$ X = \sqrt{s^2 + s^2} = \sqrt{2}s \text{ by the Pythagorean Theorem} $$

Therefore,

    $$ X = \sqrt{2}\sqrt{\frac{\pi}{4}}D_{eff} = \sqrt{\frac{\pi}{2}}D_{eff} $$
