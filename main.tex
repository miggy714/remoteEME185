...
\label{sec:system_architecture}
    ...
    % Paragraph 1: Executive Summary of Subsection

    The following section is characterized by three elements: system requirements, system architecture overview, and lastly a subsystem overview. Our selection criteria is goverened by our system requriements, and every decisions we made cascades as such.
    ...
    \label{sec:sys_arch_overview}
        ...
        \begin{figure}[H]
            ...
            \fbox{\parbox[c][3in]{0.85\textwidth}
                ...
                \begin{tikzpicture}[
                    ...
                    \node(diam_output)  [right = of valve]      {$\Delta D_{\text{effective}} = k_2 \Delta X$};

                    \draw[->] (temp_input) -- (actuator);
                    \draw[->] (valve) -- (diam_output);
                    \draw[->] (actuator) to node[right] {$\Delta L = k_1 \Delta T$} (interface) ;
                    \draw[->] (interface) to node[right] {$\Delta X =  \Delta L$} (valve);
        ...
        \todo{Update the above with this}

        \begin{figure}[H]
            \centering
            \includegraphics[height=2in]{controls_diagram.png}
