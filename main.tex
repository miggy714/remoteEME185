...
\newcommand{\specCrossSectArea}{0.002827 m^2}
\newcommand{\specOrigActuatorLength}{84.132 mm}
...
{%
    \begin{tabular}{|m{0.45\textwidth}|m{0.5\textwidth}|}
        ...
        \vspace{0pt}
            ...
            to within \dDdXTol
                &
            \todo{Add reference to a dimension tolerance table. Probably in Appendix.(Sam progress)} \\
...
{%
    \begin{tabular}{|m{0.45\textwidth}|m{0.5\textwidth}|}
        ...
        &
        \todo{Add references to dimension tolerances listed somewhere in Appendix(Sam progress)}
        \\ \cline{2-2}
        &
...
\label{sec:system_architecture}
    ...
    \label{sec:initial_system_requirements}
        ...
        off of the area of the opening. We find the area of the opening and then use the formula $D_{\text{effective}}=\sqrt{\frac{4A}{\pi}}$ 
...
the valve. The net axial displacement delivered to the valve can be expressed as

\begin{equation}
    \delta_{net} = \delta_{actuator} - \delta_{mount} = \alpha_{act} L_{act} \Delta T - \alpha_{mount} L_{mount} \Delta T
    \label{eq:net_displacement}
\end{equation}

where $\alpha_{act}$ and $\alpha_{mount}$ are the coefficients of thermal 
expansion of the actuator and mount materials respectively, $L_{act}$ and 
$L_{mount}$ are their respective reference lengths, and $\Delta T$ is the 
temperature change from the reference state. To maximize $\delta_{net}$, 
the mount material should minimize $\alpha_{mount}$ relative to $\alpha_{act}$.
        ...
        \end{figure}
            ...
            % Paragraph 5
            Since the mount and fixture sit entirely outside the pressure boundary and are never exposed to the working gas, hydrogen compatibility is not a driving material constraint for these components. The selection of Invar 36 is governed by the fact that it has a very low coefficient of thermal expansion. This results in a much lower error as the mount is expanding opposite of the actuator and the more it expands, the more error will be induced. Invar also has relatively good machinability compared to other materials with low CTE and it also has pretty good structural stiffness. The mount is important because it must be stiff enough that its deflection under the actuator's thermal forces is negligible compared 
            to the 267 $\mu$m total actuator stroke. The cross-sectional geometry of the trapezoidal plates coupled with the stiffness of Invar 36 (140 GPa), satisfies this requirement. 
...
expansion and the resisting forces have negligible effect on rod length.
        ...
        \label{subsec:act_spec_4}
            ...
            \todo{Insert discussion of mechanical envelope and reasonable foray outside of it(Ask Isaiah)}   
    ...
    \section{Mount Subsystem}
        ...
        \label{subsec:mount_spec_1}
            ...
            Error prop reference here: \ref{app:errorProp}
        ...
        \label{subsec:mount_spec_3}
            ...
            \todo{(JBandJP)Update the numbers in this section to reflect:
                \begin{itemize}
        ...
        \subsection{Trapezoidal Mount Plate Current Implementation}
            The two trapezoidal mount plates are machined from Invar 36 nickel-iron alloy. The plates rise vertically from the ficture and taper inward toward the top, forming the A-fram geometry visible in figure xyz. The actuator rod fits in the cavity between the two plates with the upper end welded to the interior
        ...
        \subsection{Trapezoidal Mount Thermal Exapnsion}
            The net displacement delivered to the valve is the difference between the actuator's thermal expansion and the mount's thermal expansion over the same vertical span. Let $L$ be the actuator rod length, which approximately equals the mount height from the actuator anchor point to the housing seat. 
            The device is calibrated at the reference temperature $T_{\text{ref}} = 25^\circ\text{C}$. The maximum one-sided displacement occurs at the cold extreme of $-60^\circ\text{C}$, giving $\Delta T = 85^\circ\text{C}$. The net stroke from the calibration point to the cold extreme is:
            \[ \delta_{\text{net}} = L \cdot \alpha_{\text{Al}} \cdot \Delta T - L \cdot \alpha_{\text{Invar}} \cdot \Delta T = L \cdot \Delta\alpha \cdot \Delta T \]
            
            Where,
            \[ \Delta\alpha = \alpha_{\text{Al}} - \alpha_{\text{Invar}} = 23.6 \times 10^{-6} - 1.30 \times 10^{-6} = 22.3 \times 10^{-6} /^\circ\text{C} \] The required stroke from \( T_{\text{ref}} \) to the cold extreme is $162 \, \mu\text{m}$ (corresponding to the change in \( D_{\text{eff}} \) from \( A_0 = 0.254 \, \text{mm} \) at $25^\circ\text{C}$ to $0.384 \, \text{mm}$ at $-60^\circ\text{C}$). 
            
            To deliver this net stroke, the actuator length must be:
            \[ L = \frac{\delta_{\text{net}}}{\Delta\alpha \cdot \Delta T} = \frac{0.162}{(22.3 \times 10^{-6} \times 85)} = \frac{0.162}{1.896 \times 10^{-3}} \approx 85.5 \, \text{mm} \]

            Invar 36 was chosen for its low CTE. If the mount were fabricated from 1018 cold-rolled steel ($\alpha = 12.0 \times 10^{-6} /^\circ\text{C}$), the differential CTE would be only $\Delta\alpha = 23.6 - 12.0 = 11.6 \times 10^{-6} /^\circ\text{C}$, 
            and the required actuator length would be: $L_{\text{steel}} = \frac{0.162}{(11.6 \times 10^{-6} \times 85)} \approx 164.4 \, \text{mm}$
            An actuator of this length would exceed the $100 \, \text{mm}$ mechanical envelope guideline even before accounting for end caps, fittings, and the valve housing beneath it. By contrast, the $85.5 \, \text{mm}$ actuator length enabled by the Invar mount fits well within the envelope.

        \subsection{Trapezoidal Mount Structural Stiffness}
            ...
            of F = 168.96 N, which is the bellow spring force at full stroke, per Section 3.3. This force is reacted at the top of the mount and transmitted down through the two trapezoidal plates to the fixture. A detailed stiffness analysis using the machined cross-section geometry from the CAD model, or equivalently a finite element analysis under the 168.96 N reaction load, is planned for the spring quarter model validation phase. 
            Given that Invar 36 has a modulus of 141 GPa and the mount plates have substantial cross-sectional area at their base, the deflection under such a modest load is expected to be negligible relative to the 162 µm stroke, but this will be quantitatively confirmed before fabrication.
        
        \subsection{Fasteners Design Requirement Updates}
            The fastener joints must maintain a sufficient enough clamping force across the full operating temperature range. Any discrepency between the fasteners and the fixture corrupts the calibration of the entire device. 

        \subsection{Fasteners Current Implementation}
            Eight M3 black-oxide alloy steel socket head screws secure the mount plates to the fixture. The valve housing and calibration shim stack are clamped between these screws. Each screw passes through the mount from the top, through the washer calibration stoack, over the housing, and threads into the fixture at the base.
            A standard 18-8 M4 stainless steel oversized washer sits under each screw head to dsitribute the clamping load across the mount plate surface. 
        ...
        \begin{table}[H]
                ...
                \begin{tabularx}{\textwidth}{l X l}
                    ...
                    \midrule
                    Fastener &
                    M3 Black-oxide alloy steel socket head screw &
                    McMaster 91290A254
                        \\\addlinespace
                    $A_t$ &
                    Tensile stress area &
        ...
        \[ F_{bolt} = A_t \times S_p \]. Plugging in the values from the table:
        \[ F_{bolt} = 14.2 mm^2 \times 9.7 \times 10^8 {N/mm^2} = 13.774 \ \text{Giganewtowns} \]

        For a total of eight screws:
         \[ F_{total} = 8 \times 14.2 mm^2 \times 9.7 \times 10^8 {N/mm^2} = 110.192 \ \text{Giganewtowns} \]
        ...
        \[ FOS_{bolt} = F_{total} / F = 6.52 \times 10^8 \]
        
        The bolted joint is is more than capable of receiving the actuator load. The M3 fastener size was selected not for the structural necessity but the for standardization, ease of assembly, and compabitibility with the shim washer stack geometry.

        \subsection{Fasteners Preload Stability}
        ...
        Given that the proof load per bolt ($13.774 \ \text{Giganewtons}$) exceeds the per-bolt share of the actuator load ($168.96 / 8 \approx 21.1 \, \text{N}$) by a factor of $6.52 \times 10^8$, this preload variation does not threaten joint integrity. 
    ...
    \label{subsec:sys_comp}
        ...
        \todo{Prove that, given worst-case deviation within specs, the whole system requirement is still met.(Unassigned)}
...
\begin{thebibliography}{99}
    ...
    Carpenter Technology Corporation, "Invar 36," material safety datasheet. \url{https://www.carpentertechnology.com/hubfs/7407324/Material%20Saftey%20Data%20Sheets/Invar%2036.pdf.}
...
\begin{table}[H]
    ...
    \begin{tabular}{|m{0.2\textwidth}|m{0.3\textwidth}|m{0.4\textwidth}|}
        ...
        Valve orifice must govern $\frac{dD_{\text{eff}}}{dX} = \sqrt{\frac{2}{\pi}}$ to within \dDdXTol\
            &
        \todo{Add reference to dimension tolerance table in Appendix(Unassigned)}
        \\ \cline{2-3}
        ...
        \multirow[c]{3}{0.3\textwidth}{%
            ...
            &
        \todo{Add reference to dimension tolerances in Appendix(Unassigned)}
        \\ \cline{3-3}
...
\includepdf[pages=1-14, fitpaper=true, landscape=true]{MachineArchDrawPackage.pdf}
...
This calculation comes from Kline-McClintock error propagation methods used in experimental probability and uncertainty propagation \cite{ref_Figliola}
