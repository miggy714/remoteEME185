...
expansion and the resisting forces have negligible effect on rod length.
        ...
        \subsubsection{Specification 4: Acceptable footprint referencing mechanical envelope}
    ...
    \section{Mount Subsystem}
     ...
     \subsection{Overview}
        ...
        See Section \ref{tab:trapmountthermalexpansion}.
        ...
        \label{tab:trapmountthermalexpansion}
        ...
        The bolted joint is is more than capable of receiving the actuator load. The M3 fastener size was selected not for the structural necessity but the for standardization, ease of assembly, and compatibility with the shim washer stack geometry.
    ...
    $$ \delta_{\text{net}} = L_{\text{act}} \cdot \alpha_{\text{Al}} \cdot \Delta T - H_{\text{mount}} \cdot \alpha_{\text{Invar}} \cdot \Delta T $$ as derived in Section 3.3.8. The selection of Invar 36 for the mount minimizes this competing expansion. At the cold extreme of $-60\,^\circ\text{C}$, the mount contributes only $\alpha_{\text{Invar}} \cdot H \cdot \Delta T = (1.2 \times 10^{-6})(88.9)(85) \approx 9.1\,\mu\text{m}$ of opposing displacement, 
    compared to the actuator's gross expansion of approximately $162\,\mu\text{m}$ over the same interval. The actuator length $L_0 = 84.335\,\text{mm}$ was sized to deliver the required net stroke after subtracting this mount expansion.

    \subsection{Actuator–Valve Interface}
    The lower end of the actuator rod is welded directly to the valve stem. This rigid connection transmits the actuator's thermal displacement to the translating orifice plate with a unity gain of $dX/dL = 1$. There is no linkage, gear train, 
    or motion amplification between the actuator and valve — the displacement delivered by the actuator is identically the displacement received by the valve plate. This direct coupling eliminates backlash and hysteresis that would arise from 
    intermediate mechanical interfaces. However, it also means that the valve's reaction force is transmitted directly back into the actuator. The bellows spring force of $F_s = 168.96\,\text{N}$ at full stroke (Section 3.1.5) compresses the 
    actuator rod axially, producing a mechanical deflection of $\delta_{\text{mech}} \approx 0.23\,\mu\text{m}$ (Section 3.2.2). This deflection is less than $0.1\%$ of the thermal stroke and is therefore negligible, but it is carried upward 
    through the actuator into the mount, which must be stiff enough to absorb it without measurable deflection of its own.

    \subsection{Mount–Valve Interface} 
    The mount sits on top of the valve housing, separated by the calibration shim stack. The vertical distance from the interior top face of the mount (where the actuator is anchored) to the orifice plate (where the translating plate contacts 
    the fixed plate) defines the reference offset $X(T_{\text{ref}})$. This distance governs the constant $A_0$ in the Required Relationship through eq. 2.7. The shim washers between the mount and housing allow this critical dimension to be 
    adjusted during final assembly to bring $X(T_{\text{ref}})$ within the specified $\pm 5\,\mu\text{m}$ of its target value (Section 3.3.2). The eight M3 fasteners clamp the mount, shim stack, and housing together as a rigid unit, ensuring 
    ...
    wall thickness ensures that the up-to-$20\,^\circ\text{C}$ temperature difference between the gas and the ambient air does not propagate to the actuator (Section 3.1.4). The valve stem, fabricated from Invar 36, provides 
    additional thermal resistance between the pressurized interior and the actuator. Its own thermal expansion of $2.45\,\mu\text{m}$ over the full temperature range (Section 3.1.2) acts in the same direction as the mount's
    expansion — opposing the actuator — but is small relative to the mount's contribution and partially offset by it.

    \subsection{Error Propagation Across Subsystems} 
    The tolerances specified for each subsystem were allocated such that the root-sum-square of all subsystem errors remains within the tolerance band of the Required Relationship. The actuator's thermal expansion error 
    of $0.2\,\mu\text{m}$ (Section 3.2.1), the mount's thermal expansion of approximately $9.1\,\mu\text{m}$ (accounted for in the actuator length calculation), the valve stem's thermal expansion of $2.45\,\mu\text{m}$, and 
    the mechanical deflection of $0.23\,\mu\text{m}$
    ...
    \label{subsec:sys_comp}
        ...
        \todo{Prove that, given worst-case deviation within specs, the whole system requirement is still met.(Sam and Isaiah)}
...
\begin{table}[H]
    ...
    \begin{tabular}{|m{0.2\textwidth}|m{0.3\textwidth}|m{0.4\textwidth}|}
        ...
        Valve orifice must govern $\frac{dD_{\text{eff}}}{dX} = -\sqrt{\frac{2}{\pi}}$ to within \dDdXTol\
            &
        \todo{Add reference to dimension tolerance table in Appendix(Sam)}
        \\ \cline{2-3}
        ...
        \multirow[c]{3}{0.3\textwidth}{%
            ...
            &
        \todo{Add reference to dimension tolerances in Appendix(Sam)}
        \\ \cline{3-3}
