...
\chapter{System Architecture Selected}
...
\label{sec:discussion}
    ...
    \section{Valve}
        ...
        \subsection{Design Requirement Updates(Roll-down)}
...
\section{Interface}
        ...
        \subsection{Design Requirement Updates(Roll-down)}
        \subsection{Proof}
            An interface must be made to transfer motion from the actuator to the valve shaft. 
            As noted in the actuator section (make sure this is mentioned) actuation distance increases with actuator base length $L_o$ (can reference equation used in prior section or appendix), 
            but the actuator size is limited by the mechanical envelope as mentioned before and it has been determined that to meet actuation needs without an amplifying interface the actuator would need to be of length $L_o$ = (insert from JBs calcs). 
            This puts our system outside of the mechanical envelope (or close to it, check with JB) and necessitates an amplifying transmission. We chose rack and pinion compound gear train as this translates linear actuator movement to rotational, amplifies it using a [1:2] compound gear set, then translates back to linear movement with a second rack attached to the valve piston. 
            Note that any error is also propagated through the train facilitating another reason to maximize actuator length and minimize need for large gear ratios. To decide on train specifications an optimization study was done to find a gear ratio that allowed for maximum precision and delivered necessary movement from the actuator to the valve.
