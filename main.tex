...
\label{sec:system_architecture}
    ...
    \label{sec:valve_sub_description}
        ...
        \label{subsec:valve_architecture}
            ...
            \todo{Reference relevant figures}
    ...
    \label{sec:act_sub_description}
        ...
        $\frac{dL}{dT}$ must equal $\sqrt{\frac{2}{\pi}} A_1$. Two sources drive its error:
...
\label{sec:discussion}
    ...
    \section{Actuator}
        ...
        The requirements for actuator operation have remained the same, though in the current implementation more 
        attention has been paid to those that are the most critical. The governing equation for the actuator expansion
        $Delta L = L_o\Delta T\alpha $ relates expansion magnitude to undeformed length $L_o$ and the linear coefficient
        of expansion $\alpha$. In preliminary designs actuator length was maximized to be within the mechanical envelope.
        An amplifying interface between the valve stem and actuator was designed to amplify expansion via 
        a gear train. This was done to meet actuation distance needs of the valve while keeping within the mechanical 
        envelope. Further calculations showed that backlash and machining inaccuracies would be compounded by the gear
        train comprimising the precision of the system as a whole. Eliminating the amplifying interface in favor of
        increasing actuator length $L_o$ is a favorable solution and it has been established that the mechanical envelope
        can be treated as a soft constraint. Though deviation from the envelope is allowed, further research was done into
        exotic materials with higher linear coefficients of expansion. Research on actuator materials has solidified
        the importance of stable coefficients of expansion, material stiffness to avoid deformation during use, and repeatable 
        performance during cyclic thermal loading. 
        \subsubsection{Subsystem Interactions}
    ...
    \section{Mount}
        ...
        \subsection{Specification 2: Critical dimension adjustment}
        \label{subsec:mount_spec_2}

            \todo{Insert details about the mount's critical dimension adjustment range. Show that it can be adjusted from within \calRange\ above the target value to within \calTol\ above or below said target}

        \subsection{Specification 3: Thermal expansion}
        \label{subsec:mount_spec_3}

            \todo{Insert details about thermal expansion of the mount. Show that thermal expansion is limited to \mountdLTol}

        \subsection{Specification 4: Mechanical envelope}
        \label{subsec:mount_spec_4}

            \todo{Briefly show that dimensions along principle axes lie within the mount's mechanical envelope. TODO: specify three-axis spatial limits for mount}        
        \subsection{Design Requirement Updates}
            ...
            % Paragraph 3
            After realizing how error can cascade from the thermal expansion of the mount, it 
        ...
        only thermal expansion in mount need to worry about is the a frame 
