        ...
        \begin{enumerate}
            \item The device must passively restrict the provided flow of gas without external input or control such that the effective diameter of its orifice obeys the following relationship with the ambient temperature of the outside environment:
                ...
                \begin{align*}
                    ...
                    \\
                    &T_{\text{ambient}}\ \epsilon\ [\specTempLow, \specTempHigh] \degC 
                \end{align*}
    ...
    \label{sec:valve_sub_description}
        ...
        \label{subsec:valve_architecture}
            ...
            \begin{itemize}
                ...
                \item Central valve assembly. Pictured in more detail in Figure \ref{fig:valve_wireframe}, the valve stem (V6) is welded to the top of the valve orifice (V5). It protrudes through a hole bored in the top of the housing center to connect to the actuator. A bellows (V6) is used to hermetically seal this hole.
            \end{itemize}
            ...
            The resulting orifice, as shown in Figure \ref{valve_interior}, is very small compared to the entire assembly.
            ...
            \begin{enumerate}
                \item The required relationship between $T_{\text{amb}}$ and $D_{\text{eff}}$ is linear.
                        ...
                        Thus, our team selected a system that will keep maintain a constant value for $\frac{dD}{dX}$.
...
expansion and the resisting forces have negligible effect on actuator length.
    ...
    \section{Mount Subsystem}
     ...
     \subsection{Overview}
        ...
        \label{subsec:mount_spec_3}
            ...
            This meets the specified deviation. Its implications are dealt with in Section \ref{sec:sys_comp}.
        ...
        The bolted joint is is more than capable of receiving the actuator load. The M5 fastener size was selected not for the structural necessity but the for standardization, ease of assembly, and compatibility with the shim washer stack geometry.
    ...
    \label{subsec:sys_comp}
        ...
        Because $D_{\text{effective}} = \sqrt{\frac{2}{\pi}}*X(T)$, the maximum error in the Required Relationship according to these specification is
...
\begin{equation}
    x(T) = D_{\text{eff}}(T) = \sqrt{\frac{\pi}{2}} \cdot \Big( A_1 (T - T_{\text{ref}}) + A_0 \Big)
\end{equation}
Propagating the error through variables $A_1$ and $A_0$:
\begin{align}
    &\theta_{A1}(T) = \frac{\partial D_{\text{eff}}}{\partial A_1} = T - T_{\text{ref}}
    \\
    &\theta_{A1}(T) = \frac{\partial D_{\text{eff}}}{\partial A_1} = T - T_{\text{ref}}
\end{align}
...
\begin{align}
    ...
    \\
    &u_D(T) = \sqrt{\Big( (T - T_{\text{ref}}) \cdot u_{A1} \Big)^2 + \big( u_{A0} \big)^2}
\end{align}
Total radial error:
\begin{equation}
    u_x(T) = \sqrt{\frac{\pi}{2}} \cdot \sqrt{\Big( (T - T_{\text{ref}}) \cdot u_{A1} \Big)^2 + \big( u_{A0} \big)^2}
\end{equation}
