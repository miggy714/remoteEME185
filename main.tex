...
\newcommand{\dXdLTol}{$0.1 \mu m$ \todo{Fix dXdLTol def to exceed thermal expansion (2.4 $\mu m$) and compression (??) of valve stem}}
...
expansion and the resisting forces have negligible effect on rod length.
    ...
    \label{subsec:sys_comp}
        ...
        Because $D_{effective} = \sqrt{\frac{2}{\pi}}*X(T)$, the maximum error in the Required Relationship according to these specification is
        $\sqrt{\frac{2}{\pi}}*(10.4 \mu m) = 8.3 \mu m$. This is within the tolerance specification for all temperatures.
        As stated in Section \ref{subsec:initial_system_requirements} and Appendix \ref{app:errorProp}, the minimum tolerance is 12.7 $\mu$m.
        This leaves enough room for unseen errors.

        Note that this calculations makes a number of first-order assumptions, which will be refined early next quarter as the computational model is optimized.
        These assumptions included the following:

        \begin{itemize}
            \item The square orifice will have a negligible fillet radius via high-precision manufacturing techniques. These methods will be researched early next quarter.
