...
\label{sec:system_architecture}
    ...
    \label{sec:valve_sub_description}
        ...
        \end{figure}
            ...
            % Paragraph 1
            The valve subsystem is a two-plate sliding orifice mechanism colloquially referred to as the "pizza cutter" valve. It is comprised by one fixed plate and one translating plate. Both contain identical square holes cut in the center. The two plates maintain a surface contact.
            ...
            % Paragraph 3
            The valve design has only one moving part, the translating plate. As stated previously, movement is driven axially by the valve stem. The valve stem passes through the top wall of the valve housing and is sealed by a welded metal bellow. Since there are no linakges, rotating components, or other types of internal mechanisms,
...
\label{sec:discussion}
    ...
    \section{Valve Subsystem}
        ...
        \label{subsec:valve_spec_3}
            ...
            The wall thickness of $10\ \text{mm}$ produces a Von Mises stress of $156.75\ \text{MPa}$, yielding a factor of safety of $FS \approx 2.55$.
    ...
    \subsection{Specification 1: Thermal Expansion Tolerance}
        ...
        and other un-modeled losses in the actuator-valve system, as well as the spring constant of
        the bellows.
...
\midrule
$L_0$           & Original rod length                        & \SI{0.0841432}{m} \\
...
\vspace{1em}
\noindent\textbf{Unit conversion for spring constant:}
\begin{equation}
...
\subsection*{1. Thermal Expansion / Contraction}
The displacement of the rod due to a temperature change $\Delta T$ is:
\begin{equation}
    \delta x = L_0 \cdot \alpha \cdot \Delta T
\end{equation}

\subsection*{2. Bellows Resisting Force}
The force exerted by the bellows spring opposing the displacement:
\begin{equation}
    F_{bellows} = k \cdot \delta x
\end{equation}

\subsection*{3. Total Resisting Force}
The total resistance the actuator must overcome:
\begin{equation}
    F_{resistance} = F_{bellows} + F_{friction}
\end{equation}

\subsection*{4. Contact Area}
The face area of the actuator over which the force is applied:
\begin{equation}
    A_{contact} = w \cdot t
\end{equation}
...
\subsection*{6. Strain}
Using Young's modulus (valid for both tension and compression in aluminum):
\begin{equation}
    \varepsilon = \frac{\sigma}{E_{Al}}
\end{equation}

\subsection*{7. Mechanical Deflection Due to Resisting Force}
The elastic deformation of the rod caused by the resisting force:
\begin{equation}
    \delta_{mech} = \varepsilon \cdot L_0
\end{equation}

\subsection*{8. Percent Error of Thermal Expansion}
The mechanical deflection as a percentage of the thermal displacement:
\begin{equation}
...
%--- BEGIN CONTENT ---
    ...
    Note that this bill of materials (BOM) is incomplete and covers only material costs. Funds for this prototype were awarded nine days before this report's due date, requiring that we update the BOM early next quarter with quotes for the advanced manufacturing required to machine some of these parts.
    Testing apparatuses will also need to be purchased. Thus, the listed final cost is significantly lower than the actual projected cost of this project.
    \todo{The above seems rather self-defensive; is there a better way to say that the total cost is not a final value?}
