...
\label{sec:discussion}
    ...
    \section{Valve Subsystem}
        ...
        \label{subsec:valve_spec_2}
                ...
                \todo{The demonstrates that the valve stem won't break given the maximum specifiec load.
        ...
        \label{subsec:valve_spec_3}
            \todo{Insert details about the valve stem here from below. Include proof that because it is made of invar 36, it will not thermally expand or be compressed more than \dXdLTol}

        \subsection{Specification 3: Pressure rating}
        \label{subsec:valve_spec_3}

            \todo{Insert housing pressure calculations from below}
            

        \subsection{Specification 4: Temperature gradient}
        \label{subsec:valve_spec_4}

            The thermal actuator is located completely outside the pipe and is fully exposed to the ambient, steady-state air. Because of this the temperature 
            gradient only goes from the inside surface of the pipe to the outside surface of the pipe, so the temperature of the actuator will not be affected by the temperature of the internal gas. The actuator is thermally isolated and only depends on the outside air temperature.
        ...
        \subsection{Orifice}
            ...
            \subsubsection{Current Implementation} 
                ...
                \todo{This is redundant; it is covered in Section \ref{subsec:valve_architecture}}
    ...
    \section{Mount Subsystem}
        ...
        The actuator in its current state is a solid aluminum rod of dimensions $L=84.3348 mm, w=30 mm, t=30 mm$
        and coefficient of expansion and elastic modulus $\alpha= 0.274 \mu m/m$ and $E = 69 GPa$.
        Its qualities allow for sufficient expansion without use of an amplifying interface, it displays
        precision actuation across the temperature range, resistance to deformation under external reaction 
        forces, and lies within tolerance of the mechanical envelope.
            \begin{table}[H]
                \centering
                \caption{Actuator Parameter Values}
                \label{tab:actuator_parameters}
